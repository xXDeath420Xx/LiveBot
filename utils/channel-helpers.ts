...
interface GuildSettingsRow extends RowDataPacket {
    avatar_upload_channel_id: string | null;
}

async function getAvatarUploadChannel(interaction: ChatInputCommandInteraction): Promise<TextChannel | null> {
    const [guildSettingsRows] = await pool.execute<GuildSettingsRow[]>(
        'SELECT avatar_upload_channel_id FROM guilds WHERE guild_id = ?',
    ...
    try {
        ...
        if (!channelPermissions.has([PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.AttachFiles])) {
            await interaction.editReply({
