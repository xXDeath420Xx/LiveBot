  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!guildSettings?.announcement_channel_id) {
        if (newAvatar || resetAvatarFlag) {
          return interaction.editReply("Please set a default announcement channel with `/setchannel` before setting/resetting a custom avatar.");
        }
      } else {
        const announcementChannel = await interaction.guild.channels.fetch(guildSettings.announcement_channel_id);
        if (!announcementChannel) {
          if (newAvatar || resetAvatarFlag) {
            return interaction.editReply("The default announcement channel seems to be deleted. Please set a new one.");
          }
        } else {
