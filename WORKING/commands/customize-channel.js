    ...
    try {
      if (newAvatarAttachment) {
        ...
        try {
          ...
          // It's crucial *not* to delete tempMessage here. The URL obtained from Discord for attachments
          // in a sent message is permanent as long as the message exists. If deleted, the URL will expire.
        } catch (uploadError) {
        ...
        if (newAvatarUrlText?.toLowerCase() === "reset" || newAvatarUrlText === "") {
          finalAvatarUrl = null; // Treat 'reset' or an empty string as a request to clear the avatar
        } else {
      ...
      // --- Start Fix for Database Query Logic ---
      ...
      // --- End Fix for Database Query Logic ---
      ...
      await interaction.editReply(`An error occurred while updating the channel customization: ${error.message}. Ensure your database table  channel_settings  is created.`);
    }
  }
};