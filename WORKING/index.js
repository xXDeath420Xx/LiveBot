...
async function main() {
  ...
  for (const file of commandFiles) {
    try {
      ...
      console.error(`[CMD Load Error] ${file}:`, e);
    }
  }
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd) {
        try {
          ...
          console.error(`Interaction Error for ${cmd.data.name}:`, e);
        }
      }
      ...
      for (const {platform, username} of requestData) {
        try {
          ...
          console.error("Error approving streamer request:", e);
        }
      }
...
async function startupCleanup(client) {
  ...
  try {
    ...
    console.error("[Startup Cleanup] A CRITICAL ERROR occurred:", e);
  }
  ...
  try {
    ...
    for (const streamer of uniqueStreamers) {
      ...
      try {
        ...
        if (primaryData && primaryData.profileImageUrl && primaryData.profileImageUrl !== streamer.profile_image_url) {
          await db.execute("UPDATE streamers SET profile_image_url = ? WHERE streamer_id = ?", [primaryData.profileImageUrl, streamer.streamer_id]);
          console.log(`[Avatar Update] Updated ${streamer.username}'s avatar.`);
        }
        ...
        if (secondaryData) {
          ...
          if (kickInfo && secondaryData.profileImageUrl && secondaryData.profileImageUrl !== kickInfo.profile_image_url) {
            await db.execute("UPDATE streamers SET profile_image_url = ? WHERE streamer_id = ?", [secondaryData.profileImageUrl, kickInfo.streamer_id]);
            console.log(`[Avatar Update] Updated linked Kick account ${streamer.kick_username}'s avatar.`);
          }
          if (secondaryData.isLive && kickInfo) {
            liveStatusMap.set(kickInfo.streamer_id, secondaryData);
          }
        }
      } catch (e) {
        ...
        try {
          ...
          if (channel) {
            await channel.messages.delete(existing.message_id).catch(err => {
              if (err.code !== 10008) {
                console.error(`[Cleanup] Failed to delete message ${existing.message_id}:`, err);
              }
            });
          }
...
async function checkTeams(client) {
  ...
  try {
    ...
    if (teamSubscriptions.length === 0) {
      console.log("[Team Sync] No teams are subscribed for syncing.");
