...
const geminiApi = __importStar(require("../utils/gemini-api.js"));
const musicMetrics = __importStar(require("./music-metrics.js"));
...
class DJManager {
    ...
    async getDJVoiceModel(guildId) {
        try {
            const [[config]] = await db_1.default.execute("SELECT dj_voice FROM music_config WHERE guild_id = ?", [guildId]);
            ...
            logger_1.default.warn(`[DJ] Failed to fetch DJ voice for guild ${guildId}, using default: ${error.message}`);
        }
    ...
    async generatePiperAudio(text, modelName, finalOutputPath) {
        ...
        return new Promise((resolve, reject) => {
            logger_1.default.info(`[DJ/Pipeline] Starting Piper -> FFmpeg audio generation for: ${finalOutputPath}`);
            const piper = (0, child_process_1.spawn)(PIPER_PATH, ['--model', modelPath, '--output-raw']);
            const ffmpeg = (0, child_process_1.spawn)(FFMPEG_PATH, ['-f', 's16le', '-ar', '22050', '-ac', '1', '-i', 'pipe:0', '-c:a', 'libopus', '-b:a', '64k', '-y', finalOutputPath]);
            ...
            piper.on('error', (err) => { logger_1.default.error(`[DJ/Piper] Failed to start Piper process: ${err.message}`); reject(err); });
            ffmpeg.on('error', (err) => { logger_1.default.error(`[DJ/FFmpeg] Failed to start FFmpeg process: ${err.message}`); reject(err); });
            ffmpeg.on('close', (code) => {
                if (code === 0) {
                    logger_1.default.info(`[DJ/FFmpeg] FFmpeg process finished successfully. Audio saved to ${finalOutputPath}`);
                    resolve(finalOutputPath);
                }
                else {
                    logger_1.default.error(`[DJ/FFmpeg] FFmpeg process exited with code ${code}. Stderr: ${ffmpegStderr}`);
                    logger_1.default.error(`[DJ/Piper] Piper Stderr (for context): ${piperStderr}`);
    ...
    async playPlaylistIntro(queue, playlistTracks, addToEnd = false) {
        logger_1.default.info(`[DJ] playPlaylistIntro triggered for guild ${queue.guild.id}.`);
        if (!playlistTracks || playlistTracks.length === 0) {
            logger_1.default.warn(`[DJ] No playlist tracks provided for intro commentary for guild ${queue.guild.id}.`);
            return;
        }
        ...
        if (queue.metadata.djMode) {
            try {
                ...
                if (!Array.isArray(playlistTracks) || playlistTracks.length === 0) {
                    logger_1.default.warn(`[DJ] Invalid playlist tracks provided for guild ${queue.guild.id}. Skipping commentary.`);
                    return;
                }
                ...
                logger_1.default.info(`[DJ] Generated intro script for guild ${queue.guild.id}: ${script}`);
                ...
                try {
                    ...
                    logger_1.default.error(`[DJ] Audio generation pipeline failed for intro commentary: ${piperError.message}. Adding tracks without intro.`);
                ...
                if (!commentarySearchResult || !commentarySearchResult.hasTracks()) {
                    logger_1.default.error(`[DJ] Failed to resolve commentary track from local file: ${audioFilePath}. Adding tracks without intro.`);
                ...
                logger_1.default.info(`[DJ] Added commentary and ${playlistTracks.length} playlist tracks.`);
            }
            catch (error) {
                logger_1.default.error(`[DJ] Failed to generate or inject intro commentary for guild ${queue.guild.id}:`, { error: error.message, stack: error.stack });
    ...
    async playSkipBanter(queue, skippedTrack, skipper) {
        ...
        try {
            ...
            if (searchResult.hasTracks()) {
                ...
                logger_1.default.info(`[DJ] Inserted skip banter for guild ${queue.guild.id}.`);
            }
        }
        catch (error) {
            logger_1.default.error(`[DJ] Failed to play skip banter for guild ${queue.guild.id}:`, { error: error.message, stack: error.stack });
        }
        finally {
            this.client.musicPanelManager?.get(queue.guild.id)?.updatePanel();
        }
    ...
    async onPlayerFinish(queue, finishedTrack) {
        logger_1.default.info(`[DJ] onPlayerFinish event triggered for guild ${queue.guild.id}. Track: ${finishedTrack.title}, Queue Size: ${queue.tracks.size}`);
        this.client.musicPanelManager?.get(queue.guild.id)?.updatePanel();
        if (finishedTrack.metadata?.isDJCommentary || finishedTrack.metadata?.isSkipBanter) {
            ...
            if (localPath && fs.existsSync(localPath)) {
                fs.unlink(localPath, (err) => {
                    if (err)
                        logger_1.default.error(`[Cleanup] Failed to delete commentary temp file: ${localPath}`, err.message);
                    else
                        logger_1.default.info(`[Cleanup] Deleted commentary temp file: ${localPath}`);
                });
            }
        }
        ...
        if (queue.tracks.size > 0 && queue.tracks.size <= 3 && queue.metadata.djMode && !queue.metadata.isGenerating) {
            logger_1.default.info(`[DJ] Queue nearing end (${queue.tracks.size} tracks left). Triggering onQueueEnd to pre-fetch next playlist.`);
            this.onQueueEnd(queue).catch((e) => logger_1.default.error(`[DJ] Error during pre-emptive onQueueEnd: ${e.message}`));
        }
        else if (queue.tracks.size === 0 && queue.metadata.djMode) {
            logger_1.default.info(`[DJ] Queue is empty after track finish. Manually triggering onQueueEnd logic.`);
            setTimeout(() => this.onQueueEnd(queue), 500);
        }
    }
    async onQueueEnd(queue) {
        logger_1.default.info(`[DJ] queueEnd event triggered for guild ${queue.guild.id}.`);
        const panel = this.client.musicPanelManager?.get(queue.guild.id);
        if (queue.metadata.isGenerating) {
            logger_1.default.warn(`[DJ] Playlist generation is already in progress for guild ${queue.guild.id}. Skipping.`);
            return;
        }
        if (queue.metadata.djMode) {
            ...
            try {
                ...
                logger_1.default.info(`[DJ] DJ mode is active. Generating new playlist. addToEnd: ${shouldAddToEnd}`);
                ...
                if (!djUser) {
                    logger_1.default.error(`[DJ] Could not fetch the DJ initiator user (ID: ${djInitiatorId}). Ending session.`);
                ...
                if (prompt && (prompt.includes('youtube.com') || prompt.includes('youtu.be'))) {
                    logger_1.default.info(`[DJ] Detected YouTube URL in prompt: ${prompt}. Extracting video info...`);
                    try {
                        ...
                        if (ytExtractor && ytExtractor.getVideoInfo) {
                            ...
                            if (videoInfo && videoInfo.title && videoInfo.author) {
                                logger_1.default.info(`[DJ] Extracted video info: "${videoInfo.title}" by "${videoInfo.author}"`);
                        ...
                        logger_1.default.error(`[DJ] Failed to extract YouTube video info: ${error.message}. Using prompt as-is.`);
                    }
                }
                ...
                if (geminiRecommendedTracks && geminiRecommendedTracks.length > 0) {
                    ...
                    for (let i = 0; i < geminiRecommendedTracks.length; i += BATCH_SIZE) {
                        ...
                        const batchPromises = batch.map(async (recTrack) => {
                        ...
                        resolvedTracks.push(...batchResults.filter((track) => track !== null));
                        logger_1.default.info(`[DJ] Processed batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(geminiRecommendedTracks.length / BATCH_SIZE)} (${resolvedTracks.length} tracks found so far)`);
                    }
