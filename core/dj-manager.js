        ...
        if (queue.metadata.djMode) {
            ...
            try {
                ...
                if (prompt && (prompt.includes('youtube.com') || prompt.includes('youtu.be'))) {
                    ...
                    try {
                        const videoInfo = await this.player.extractors.get('com.certifried.playwright-youtube').getVideoInfo(prompt);
                ...
                if (geminiRecommendedTracks && geminiRecommendedTracks.length > 0) {
                    ...
                    for (let i = 0; i < geminiRecommendedTracks.length; i += BATCH_SIZE) {
                        ...
                        const batchPromises = batch.map(async (recTrack) => {
                            ...
                            const searchResult = await this.player.search(query, { searchEngine: 'com.certifried.playwright-youtube', requestedBy: djUser, metadata: { artist: recTrack.artist } });
                            if (searchResult.hasTracks()) {
