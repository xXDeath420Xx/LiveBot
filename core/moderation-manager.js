...
async function logInfraction(interaction, user, type, reason, durationMinutes = null) {
    try {
        ...
        const [[config]] = await db.execute('SELECT mod_log_channel_id FROM moderation_config WHERE guild_id = ?', [interaction.guild.id]);
        ...
        logger.error('[ModerationManager] Failed to log infraction:', error);
    }
}

module.exports = { logInfraction };