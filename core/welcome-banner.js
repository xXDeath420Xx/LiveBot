...
exports.generateWelcomeBanner = generateWelcomeBanner;
 ...
 */
async function generateWelcomeBanner(member, backgroundUrl = null) {
    try {
        // Canvas dimensions
        const width = 1024;
        const height = 450;
        ...
        if (backgroundUrl) {
            try {
                const background = await (0, canvas_1.loadImage)(backgroundUrl);
                ...
                logger_1.default.warn(`[Welcome Banner] Failed to load background image: ${err.message}. Using default gradient.`);
        ...
        try {
            ...
            logger_1.default.error(`[Welcome Banner] Failed to load avatar for ${member.user.tag}: ${err.message}`);
        }
        // Text setup
        ...
        logger_1.default.error('[Welcome Banner] Failed to generate banner:', error);
        throw error;
    }
}
