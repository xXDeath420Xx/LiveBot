...
async function processNewMember(member) {
    ...
    try {
        ...
        if (config === undefined) {
            const [[dbConfig]] = await db.execute('SELECT * FROM join_gate_config WHERE guild_id = ?', [guildId]);
        ...
        if (config.verification_enabled) {
            const [[welcomeConfig]] = await db.execute('SELECT channel_id FROM welcome_settings WHERE guild_id = ?', [guildId]);
