...
async function checkStreams(client) {
    ...
    try {
        ...
        const [announcementsInDb] = await db.execute(`
            ...
            JOIN (
                SELECT subscription_id, MAX(announcement_id) as max_announcement_id
                FROM announcements
                GROUP BY subscription_id
            ) a2 ON a1.subscription_id = a2.subscription_id AND a1.announcement_id = a2.max_announcement_id
        `);
        ...
        for (const streamer of uniqueStreamers) {
            try {
                ...
                const liveData = await fetchAndCache(cacheKey, async () => {
                    ...
                    if (streamer.platform === 'kick') return apiChecks.checkKick(cycleTLS, streamer.username);
        ...
        for (const sub of subscriptions) {
            ...
            if (!guildSettings || !guildSettings.announcement_channel_id) continue;
...
async function checkTeams(client) {
    ...
    finally { isCheckingTeams = false; logger.info('[Team Sync] ---> Finished team sync.'); }
}
