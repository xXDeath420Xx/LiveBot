...
exports.checkStatroles = checkStatroles;
const db_1 = __importDefault(require("../utils/db"));
const logger_1 = __importDefault(require("../utils/logger"));
async function checkStatroles(client) {
    logger_1.default.info('[StatroleManager] Starting periodic check for activity-based roles.', { category: 'statroles' });
    try {
        const [configs] = await db_1.default.execute('SELECT * FROM statroles_config');
        if (configs.length === 0)
            return;
        for (const config of configs) {
            ...
            const [activity] = await db_1.default.execute('SELECT user_id, SUM(count) as total_activity FROM activity_logs WHERE guild_id = ? AND type = ? AND timestamp >= ? GROUP BY user_id', [config.guild_id, activityType, sinceDate]);
            ...
            for (const member of members.values()) {
                ...
                if (userActivity >= requiredCount && !hasRole) {
                    ...
                    logger_1.default.info(`Assigned statrole ${role.name} to ${member.user.tag} in ${guild.name}.`, { category: 'statroles' });
                }
                // Remove role if they no longer meet the threshold and have it
                    ...
                    logger_1.default.info(`Removed statrole ${role.name} from ${member.user.tag} in ${guild.name}.`, { category: 'statroles' });
                }
            }
        }
    }
        ...
        if (error.code !== 'ER_NO_SUCH_TABLE') {
            logger_1.default.error('[StatroleManager] Error checking stat roles:', error);
        }
    }
    logger_1.default.info('[StatroleManager] Finished periodic check.', { category: 'statroles' });
}
