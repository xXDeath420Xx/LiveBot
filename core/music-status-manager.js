...
import { safeJsonObject, safeJsonArray } from '../utils/safeJson.js';
...
class MusicStatusManager {
    ...
    async removeMusicSession(guildId) {
        try {
            ...
            await pool.execute('DELETE FROM music_sessions WHERE guild_id = ?', [guildId]);
    ...
    async logTrackPlay(guildId, track, userId) {
        try {
            await pool.execute(`INSERT INTO music_statistics (guild_id, track_title, track_url, track_source, played_by, played_at, duration) VALUES (?, ?, ?, ?, ?, NOW(), ?)`,
    ...
    async getAllSessions() {
        try {
            const [sessions] = await pool.execute('SELECT * FROM music_sessions ORDER BY updated_at DESC');
            const enriched = await Promise.all(sessions.map(async (session) => {
                try {
                    ...
                    return {
                        ...
                        current_track: safeJsonObject(session.current_track, 'music session current_track'),
                        filters: safeJsonArray(session.filters, 'music session filters')
                    };
                } catch (err) {
                    return null;
                }
    ...
    async getMusicStatistics(guildId = null) {
        try {
            ...
            const [stats] = await pool.execute(`SELECT COUNT(*) as total_plays, COUNT(DISTINCT guild_id) as unique_guilds, COUNT(DISTINCT played_by) as unique_users, SUM(duration) as total_duration FROM music_statistics ${whereClause}`, params);
            const [topTracks] = await pool.execute(`SELECT track_title, track_source, COUNT(*) as play_count FROM music_statistics ${whereClause} GROUP BY track_title, track_source ORDER BY play_count DESC LIMIT 10`, params);
