...
const twitchApi = tslib_1.__importStar(require("../utils/twitch-api"));
const kickApi = tslib_1.__importStar(require("../utils/kick-api"));
const facebookApi = tslib_1.__importStar(require("../utils/facebook-api"));
const instagramApi = tslib_1.__importStar(require("../utils/instagram-api"));
const youtubeApi = tslib_1.__importStar(require("../utils/youtube-api"));
const tiktokApi = tslib_1.__importStar(require("../utils/tiktok-api"));
const trovoApi = tslib_1.__importStar(require("../utils/trovo-api"));
const logger_1 = tslib_1.__importDefault(require("../utils/logger"));
const announcement_queue_1 = require("../jobs/announcement-queue");
const offline_queue_1 = require("../jobs/offline-queue");
...
async function loadAnnouncementsFromDatabase() {
    ...
    try {
        ...
        logger_1.default.info(`Loaded ${rows.length} active announcements from database`, { category: "streams" });
    }
    catch (error) {
        ...
        if (error.code === 'ER_NO_SUCH_TABLE') {
            ...
            logger_1.default.error('Failed to load announcements from database:', { error, category: "streams" });
        }
    }
    finally {
        if (connection)
...
async function saveAnnouncementToDatabase(guildId, platform, username, channelId, messageId, streamerId, discordUserId, streamStartedAt = null) {
    let connection;
    try {
        connection = await db_1.default.getConnection();
        await connection.query(`INSERT INTO live_announcements (guild_id, platform, username, channel_id, message_id, streamer_id, discord_user_id, stream_started_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        if (error.code !== 'ER_NO_SUCH_TABLE') {
            logger_1.default.error('Failed to save announcement to database:', { error, guildId, platform, username, category: "streams" });
        }
    }
    finally {
        if (connection)
...
async function deleteAnnouncementFromDatabase(guildId, platform, username, channelId) {
    ...
    try {
        connection = await db_1.default.getConnection();
        await connection.query('DELETE FROM live_announcements WHERE guild_id = ? AND platform = ? AND username = ? AND channel_id = ?', [guildId, platform, username, channelId]);
    }
    catch (error) {
        if (error.code !== 'ER_NO_SUCH_TABLE') {
            logger_1.default.error('Failed to delete announcement from database:', { error, guildId, platform, username, category: "streams" });
        }
    }
    finally {
        if (connection)
...
async function clearAllAnnouncements() {
    ...
    try {
        ...
        if (error.code !== 'ER_NO_SUCH_TABLE') {
            logger_1.default.error('Failed to clear announcements from database:', { error, category: "streams" });
        }
    ...
    try {
        ...
        for (const g of allGuilds) {
            ...
            const members = await guild.members.fetch().catch(() => {
                logger_1.default.warn(`Failed to fetch members for guild ${guild.id}.`, { guildId: guild.id, category: "streams" });
                return new Map();
            });
        ...
        logger_1.default.info(`Found ${membersToProcess.size} unique members with live roles across all guilds for cleanup check.`);
        ...
        for (const { member, guilds } of membersToProcess.values()) {
            try {
                    ...
                    for (const [guildId, roleIds] of guilds.entries()) {
                        ...
                        for (const roleId of roleIds) {
                            if (guildMember.roles.cache.has(roleId)) {
                                ...
                                if (roleToRemove) {
                                    logger_1.default.info(`Removing stale role '${roleToRemove.name}' from ${guildMember.user.tag} in guild ${guild.name}.`, { guildId: guild.id, userId: guildMember.id, category: "streams" });
                                    await guildMember.roles.remove(roleToRemove).catch(e => logger_1.default.error(`Failed to remove role ${roleId} from member ${guildMember.id} in guild ${guild.id}`, { error: e }));
                                }
                            }
                        }
                    }
                ...
                logger_1.default.error(`Error processing member ${member.id} in global cleanup`, { error: e, userId: member.id, category: "streams" });
            }
        }
    }
    catch (error) {
        logger_1.default.error("Critical _error during aggressive cleanup.", { _error, category: "streams" });
    }
    finally {
        if (connection)
            connection.release();
        ...
        for (const channelId of allChannelIds) {
            try {
                ...
                if (botMessages.size > 0) {
                    ...
                    logger_1.default.info(`Purged ${botMessages.size} old announcements from #${channel.name}.`, { category: "streams" });
                }
            }
            catch (e) {
                logger_1.default.warn(`Could not purge announcements from channel ${channelId}. It may no longer exist or permissions are missing.`, { error: e.message, category: "streams" });
            }
        }
    }
    catch (error) {
        logger_1.default.error("Critical _error during announcement purge.", { _error, category: "streams" });
    }
    finally {
        if (connection)
            connection.release();
...
async function checkStreamers(client) {
    ...
    if (shouldUpdateEmbeds) {
        logger_1.default.info(`[Stream Manager] Pass #${streamCheckPassCounter}: Will update embeds for live streamers`, { category: "streams" });
        streamCheckPassCounter = 0; // Reset counter
    }
    else {
        logger_1.default.info(`[Stream Manager] Pass #${streamCheckPassCounter}/${EMBED_UPDATE_INTERVAL}`, { category: "streams" });
    }
    let connection;
    try {
        connection = await db_1.default.getConnection();
        ...
        logger_1.default.info(`[Stream Manager] Loaded ${subscriptions[0].length} subscriptions, ${teams[0].length} teams, ${guildSettings[0].length} guilds`, { category: "streams" });
        ...
        logger_1.default.info(`[Stream Manager] Checking ${streamersToCheck.size} unique streamers now...`, { category: "streams" });
        const roleOperationsByGuild = new Map();
        let processedCount = 0;
        for (const { streamerInfo, subscriptions } of Array.from(streamersToCheck.values())) {
            processedCount++;
            logger_1.default.info(`[Stream Manager] Processing streamer ${processedCount}/${streamersToCheck.size}: ${streamerInfo.username} (${streamerInfo.platform})`, {
                category: "streams",
            ...
            logger_1.default.info(`[Stream Manager] Completed processing streamer ${processedCount}/${streamersToCheck.size}: ${streamerInfo.username}`, {
                category: "streams",
                progress: `${processedCount}/${streamersToCheck.size}`
            });
        }
        logger_1.default.info(`[Stream Manager] Finished checking all ${processedCount} streamers`, { category: "streams" });
                ...
                for (const [roleId, { shouldHaveRole, member, streamerName }] of roleMap.entries()) {
                    try {
                        ...
                        if (shouldHaveRole && !hasRole) {
                            await member.roles.add(liveRole).catch(e => logger_1.default.error(`Failed to add role:`, { error: e, guildId: guild.id, memberId: member.id, roleId: liveRole.id, category: "streams" }));
                            logger_1.default.info(`Added live role '${liveRole.name}' to ${member.user.tag}`, { guildId: guild.id, userId: member.id, streamer: streamerName, category: "streams" });
                        }
                        else if (!shouldHaveRole && hasRole) {
                            await member.roles.remove(liveRole).catch(e => logger_1.default.error(`Failed to remove role:`, { error: e, guildId: guild.id, memberId: member.id, roleId: liveRole.id, category: "streams" }));
                            logger_1.default.info(`Removed live role '${liveRole.name}' from ${member.user.tag}`, { guildId: guild.id, userId: member.id, category: "streams" });
                        }
                    }
                    catch (error) {
                        logger_1.default.error(`Error applying role changes for user ${discordUserId}:`, { _error, guildId, category: "streams" });
                    }
                }
            }
        }
        ...
        logger_1.default.error("Failed to run stream checker process:", { _error, category: "streams" });
    }
    finally {
        if (connection)
            connection.release();
...
async function processUniqueStreamer(client, streamer, subscriptions, guildsMap, teamsMap, roleOperationsByGuild, shouldUpdateEmbeds = false) {
    logger_1.default.info(`[processUniqueStreamer] ENTRY: Processing ${streamer.username} on ${streamer.platform}`, {
        category: "streams",
    ...
    const api = platformModules[streamer.platform];
    if (!api) {
        logger_1.default.error(`[processUniqueStreamer] No API module found for platform: ${streamer.platform}`, {
            category: "streams",
            platform: streamer.platform,
    ...
    logger_1.default.info(`[processUniqueStreamer] API module found, checking if ${streamer.username} is live...`, {
        category: "streams",
        apiType: typeof api,
    ...
    try {
        logger_1.default.info(`[processUniqueStreamer] Calling api.isStreamerLive for ${streamer.username}...`, { category: "streams" });
        const isLive = await api.isStreamerLive(streamer.username);
        logger_1.default.info(`[processUniqueStreamer] isStreamerLive returned: ${isLive} for ${streamer.username}`, {
            category: "streams",
            isLive,
            streamer: streamer.username,
            platform: streamer.platform
        ...
        let streamData = null;
        if (isLive) {
            logger_1.default.info(`[processUniqueStreamer] Streamer ${streamer.username} is LIVE, fetching details...`, { category: "streams" });
            streamData = await api.getStreamDetails(streamer.username);
            logger_1.default.info(`[processUniqueStreamer] getStreamDetails returned for ${streamer.username}:`, {
                category: "streams",
                hasData: !!streamData,
            ...
            if (!streamData) {
                logger_1.default.warn(`[Stream Manager] Streamer ${streamer.username} on ${streamer.platform} reported live but getStreamDetails returned null. Skipping announcement.`, { category: "streams" });
            }
        }
        else {
            logger_1.default.info(`[processUniqueStreamer] Streamer ${streamer.username} is NOT live`, { category: "streams" });
        }
        const guildOperations = new Map();
        logger_1.default.info(`[processUniqueStreamer] Processing ${subscriptions.length} subscriptions for ${streamer.username}`, {
            category: "streams",
            subscriptionCount: subscriptions.length
        });
        for (const sub of subscriptions) {
            const guildDefault = guildsMap.get(sub.guild_id);
            logger_1.default.info(`[processUniqueStreamer] Processing subscription for guild ${sub.guild_id}`, {
                category: "streams",
            ...
            if (!guildDefault) {
                logger_1.default.warn(`[processUniqueStreamer] No guild default found for ${sub.guild_id}, skipping`, {
                    category: "streams"
                });
                continue;
            }
            ...
            logger_1.default.info(`[processUniqueStreamer] Subscription channel resolution for ${streamer.username}`, {
                category: "streams",
                finalChannelId,
                finalRoleId,
                subChannelId: sub.sub_channel_id,
        ...
        logger_1.default.info(`[processUniqueStreamer] Starting guild operations for ${streamer.username}`, {
            category: "streams",
        ...
        for (const [guildId, guildOp] of guildOperationsArray) {
            ...
            logger_1.default.info(`[processUniqueStreamer] Processing guild ${guildId} for ${streamer.username}`, {
                category: "streams",
                guildId,
            ...
            for (const [channelId, { deleteOnEnd, webhookConfig, subscription, team }] of channelAnnouncementsArray) {
                ...
                logger_1.default.info(`[processUniqueStreamer] Processing channel ${channelId} for ${streamer.username}`, {
                    category: "streams",
                    channelId,
                    isLive,
                ...
                if (isLive && streamData) {
                    logger_1.default.info(`[processUniqueStreamer] Streamer is LIVE - queuing announcement job for ${streamer.username}`, {
                        category: "streams",
                        hasAnnouncementData: !!announcementData,
                        channel: channelId,
                        guild: guild.id
                    ...
                    try {
                        ...
                        await announcement_queue_1.announcementQueue.add('announcement', {
                            sub: {
                        ...
                        logger_1.default.info(`[processUniqueStreamer] Queued announcement job for ${streamer.username}`, {
                            category: "streams",
                            queuedFor: 'announcement-queue'
                        });
                    }
                    catch (queueError) {
                        logger_1.default.error(`[processUniqueStreamer] Failed to queue announcement for ${streamer.username}:`, {
                    ...
                    if (announcementData) {
                        ...
                        logger_1.default.info(`[processUniqueStreamer] Streamer is OFFLINE - queuing offline job for ${streamer.username}`, {
                            category: "streams",
                            hasAnnouncementData: !!announcementData,
                            channel: channelId,
                            guild: guild.id,
                        ...
                        try {
                            ...
                            await offline_queue_1.offlineQueue.add('offline', {
                                subscription_id: subscription.subscription_id,
                                streamer_id: streamer.streamer_id,
                                guild_id: guild.id,
                                username: streamer.username,
                            ...
                            logger_1.default.info(`[processUniqueStreamer] Queued offline job for ${streamer.username}`, {
                                category: "streams",
                            ...
                            logger_1.default.error(`[processUniqueStreamer] Failed to queue offline job for ${streamer.username}:`, {
                                error: queueError.message,
                                stack: queueError.stack,
                                guildId: guild.id,
                                channelId,
        ...
        logger_1.default.error(`Error processing unique streamer ${streamer.username} on ${streamer.platform}:`, {
...
async function init(client) {
    logger_1.default.info("Initializing aggressive stream manager...");
