import { GuildMember, MessageReaction, User, PartialMessageReaction, PartialUser, Role } from 'discord.js';
...
async function processRole(member: GuildMember | null, roleIds: string[], action: 'add' | 'remove', guildId: string): Promise<void> {
    ...
    try {
        ...
        if (error.code === 10007) { // Unknown Member
            ...
            logger.error(`An unexpected error occurred while processing roles for ${member.user.tag}.`, { guildId, category: 'role-manager', error: errorMessage });
        }
    }
}

async function cleanupInvalidRole(guildId: string, roleId: string): Promise<void> {
    ...
    try {
        ...
        logger.error(`[RoleCleanup] Failed to purge invalid role ${roleId} for guild ${guildId}:`, { guildId, category: 'role-manager', error: errorMessage });
    }
}
...
async function handleReactionAdd(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser): Promise<void> {
    ...
    try {
        const [[panel]] = await db.execute<ReactionRolePanel[]>('SELECT * FROM reaction_role_panels WHERE message_id = ?', [message.id]);
        if (!panel) return;

        const emojiIdentifier = reaction.emoji.id || reaction.emoji.name;
        const [[mapping]] = await db.execute<ReactionRoleMapping[]>('SELECT role_id FROM reaction_role_mappings WHERE panel_id = ? AND emoji_id = ?', [panel.id, emojiIdentifier]);
        if (!mapping) return;
        ...
        if (panel.panel_mode === 'unique') {
            const [allMappings] = await db.execute<ReactionRoleMapping[]>('SELECT role_id FROM reaction_role_mappings WHERE panel_id = ?', [panel.id]);
