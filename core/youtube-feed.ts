...
interface YouTubeFeed extends RowDataPacket {
    id: number;
...
async function checkYouTubeFeeds(): Promise<void> {
    try {
        const [feeds] = await db.execute<YouTubeFeed[]>('SELECT * FROM youtube_feeds');
        ...
        logger.error('[YouTubeFeed] Error checking YouTube feeds:', err as Record<string, any>);
    }
}

async function processYouTubeFeed(feed: YouTubeFeed): Promise<void> {
    try {
        ...
        const latestVideo: VideoEntry = Array.isArray(entries) ? entries[0] : entries;
        ...
        if (latestVideoId && latestVideoId !== feed.last_video_id) {
            ...
            if (!channelName) {
                ...
                await db.execute<ResultSetHeader>('UPDATE youtube_feeds SET channel_name = ? WHERE id = ?', [channelName, feed.id]);
            }
            ...
            await db.execute<ResultSetHeader>('UPDATE youtube_feeds SET last_video_id = ? WHERE id = ?', [latestVideoId, feed.id]);
        }
    } catch (error) {
        const err = _error as Error;
        logger.warn(`[YouTubeFeed] Failed to process feed for channel ID ${feed.youtube_channel_id}:`, err.message);
    }
}
