...
interface CustomCommand extends RowDataPacket {
    ...
    command_name: string;
    response: string;
    action_type: 'reply' | 'add_role' | 'remove_role';
...
async function handleCustomCommand(interaction: ChatInputCommandInteraction): Promise<boolean> {
    if (!interaction.isChatInputCommand()) return false;

    const guildId = interaction.guild!.id;
    const commandName = interaction.commandName.toLowerCase();
    const cacheKey = `${guildId}:${commandName}`;
    let command: CustomCommand | undefined;

    if (commandCache.has(cacheKey)) {
        command = commandCache.get(cacheKey);
    } else {
        const [rows] = await db.execute<CustomCommand[]>('SELECT * FROM custom_commands WHERE guild_id = ? AND command_name = ?', [guildId, commandName]);
        if (rows.length > 0) {
            command = rows[0];
    ...
    if (command) {
        try {
            // Permission Checks
            const requiredRoles: string[] = command.required_roles ? JSON.parse(command.required_roles) : [];
            if (requiredRoles.length > 0 && !(interaction.member as GuildMember).roles.cache.some(r => requiredRoles.includes(r.id))) {
                return interaction.reply({ content: 'You do not have the required role to use this command.', ephemeral: true }).then(() => true);
            }

            const allowedChannels: string[] = command.allowed_channels ? JSON.parse(command.allowed_channels) : [];
            ...
            if (error.code !== 'ER_BAD_FIELD_ERROR') {
                logger.error(`[CustomCommandHandler] Error executing command '${commandName}':`, error as Record<string, any>);
            }
...
function parseVariables(text: string, interaction: ChatInputCommandInteraction): string {
    if (!text) return '';
    ...
    const replacements: Record<string, string> = {
        ...
        '{channel.name}': interaction.channel!.isDMBased() ? 'DM' : (interaction.channel as any).name,
...
function invalidateCommandCache(guildId: string, commandName: string): void {
...
export { handleCustomCommand, invalidateCommandCache };
