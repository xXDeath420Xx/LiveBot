...
class PreloadManager {
    ...
    private preloadedPages: Map<string, PreloadedData>;
    private currentPreloads: Map<string, Promise<any>>;
    private maxPreloadCount: number;
    private maxPreloadAge: number;
     ...
     */
    async preloadMultiple(tracks: Track[]): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    async preloadNext(track: Track): Promise<any> {
     ...
     * @private
     */
    async _prepareStream(track: Track): Promise<{ page: Page; info: any; preparedData: PreparedData } | null> {
        const info = {
            title: track.title,
            url: track.url,
        ...
        try {
            ...
            const preparedData = await page.evaluate(async (expectedDurationSeconds: number) => {
     ...
     */
    async consumePreloaded(info: any): Promise<PreloadedData | null> {
