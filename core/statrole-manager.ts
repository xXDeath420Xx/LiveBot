...
async function checkStatroles(client: Client): Promise<void> {
    logger.info('[StatroleManager] Starting periodic check for activity-based roles.', { category: 'statroles' });
    try {
        const [configs] = await db.execute<StatroleConfig[]>('SELECT * FROM statroles_config');
        ...
        for (const config of configs) {
            ...
            const [activity] = await db.execute<ActivityLog[]>(
        ...
        if (error.code !== 'ER_NO_SUCH_TABLE') {
            logger.error('[StatroleManager] Error checking stat roles:', error as Record<string, any>);
        }
    }
    logger.info('[StatroleManager] Finished periodic check.', { category: 'statroles' });
}

export { checkStatroles };
