...
async function handleInteraction(interaction) {
    ...
    try {
        if (interaction.isChatInputCommand()) {
            ...
            else logger.warn(`No button handler found for custom ID: ${interaction.customId}`);
        } else if (interaction.isModalSubmit()) {
            const handler = findHandler(interaction.client.modals, interaction.customId);
            if (handler) await handler.execute(interaction);
            else logger.warn(`No modal handler found for custom ID: ${interaction.customId}`);
        } else if (interaction.isStringSelectMenu() || interaction.isChannelSelectMenu()) {
            const handler = findHandler(interaction.client.selects, interaction.customId);
            if (handler) await handler.execute(interaction);
            else logger.warn(`No select menu handler found for custom ID: ${interaction.customId}`);
        }
    } catch (error) {
