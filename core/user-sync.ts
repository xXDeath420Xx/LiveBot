...
import { normalizeUsername } from '../utils/db';
...
interface UserMissingKick extends RowDataPacket {
    discord_user_id: string;
    username: string;
    normalized_username: string;
}

interface ExistingKickRow extends RowDataPacket {
    streamer_id: string;
}
 ...
 */
async function syncDiscordUserIds(client: Client): Promise<void> {
    ...
    try {
        ...
        for (const guild of guilds) {
            try {
                ...
                logger.warn(`[UserSync-P1] Could not fetch members for guild ${guild.name}: ${error.message}`);
            }
        }
        ...
        const [allStreamers] = await db.execute<StreamerRow[]>('SELECT streamer_id, username, platform, discord_user_id, normalized_username FROM streamers');
        const normalizedGroups = new Map<string, StreamerRow[]>();
        ...
        for (const [normalized, group] of normalizedGroups.entries()) {
            ...
            if (masterId && needsUpdate) {
                ...
                try {
                    const [result] = await db.execute<ResultSetHeader>(`UPDATE streamers SET discord_user_id = ? WHERE streamer_id IN (${placeholders})`, [masterId, ...idsToUpdate]);
        ...
        const [usersMissingKick] = await db.execute<UserMissingKick[]>(`
        ...
        if (usersMissingKick.length === 0) {
            ...
            for (const user of usersMissingKick) {
                ...
                const [existingKickByNormalizedName] = await db.execute<ExistingKickRow[]>("SELECT streamer_id FROM streamers WHERE platform = 'kick' AND normalized_username = ?", [normalized_username]);
                ...
                try {
                    ...
                    const err = kickError as Error;
                    logger.warn(`[UserSync-P2] Error checking Kick for username ${username}: ${err.message}`);
                }
            }
        }
        ...
        logger.error('[UserSync] CRITICAL ERROR during comprehensive sync:', { error: err.stack  });
    }
}

export = { syncDiscordUserIds };
