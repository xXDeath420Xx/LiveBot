...
async function handleMemberJoin(member) {
    ...
    try {
        ...
        logger.error('Error in anti-raid handler.', { guildId, category: 'anti-raid', error: error.stack });
    }
}
...
async function takeAction(guild, raiders, action) {
    ...
    try {
        ...
        const [[modConfig]] = await db.execute('SELECT mod_log_channel_id FROM moderation_config WHERE guild_id = ?', [guildId]);
        ...
        for (const userId of userIds) {
            ...
            try {
                ...
                logger.error(`Failed to ${action} member ${member.id}.`, { guildId, category: 'anti-raid', error: e.stack });
            }
        }
        ...
        logger.error('Error taking anti-raid action.', { guildId, category: 'anti-raid', error: error.stack });
    }
}
