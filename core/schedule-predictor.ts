...
interface StreamerInfo extends RowDataPacket {
    username: string;
    platform: string;
}

interface HistoricalData extends RowDataPacket {
    created_at: Date;
    updated_at: Date;
}
...
interface CachedPrediction extends RowDataPacket {
    prediction_data: string;
    analyzed_streams_count: number;
    created_at: Date;
    expires_at: Date;
    username: string;
    platform: string;
 ...
 */
async function predictStreamerSchedule(streamerId: number, daysToAnalyze: number = 30): Promise<PredictionResult> {
    if (!GEMINI_API_KEY || !model) {
        logger.error('[Schedule Predictor] Cannot predict schedule: Gemini API key is missing.');
    ...
    try {
        ...
        const [[streamerInfo]] = await db.execute<StreamerInfo[]>(
            'SELECT username, platform FROM streamers WHERE streamer_id = ?',
            [streamerId]
        );
        ...
        const [historicalData] = await db.execute<HistoricalData[]>(`
        ...
        const streamTimes: StreamTime[] = historicalData.map(record => {
...
Be analytical and data-driven. If the data shows no clear pattern, say so in the consistency field.`;
        ...
        logger.info(`[Schedule Predictor] Analyzing ${streamTimes.length} historical streams for ${streamerInfo.username}`);
        ...
        const prediction: Prediction = JSON.parse(text);
        ...
        logger.info(`[Schedule Predictor] Successfully predicted schedule for ${streamerInfo.username}`);
        ...
        logger.error('[Schedule Predictor] Error predicting schedule:', { error: error.message, stack: error.stack });
 ...
 */
async function getCachedPrediction(streamerId: number): Promise<any> {
    try {
        const [[cached]] = await db.execute<CachedPrediction[]>(`
        ...
        logger.error('[Schedule Predictor] Error fetching cached prediction:', error as Record<string, any>);
        return null;
    }
}

export { predictStreamerSchedule, getCachedPrediction };
