...
async function restoreBackup(backupId, client) {
    let guildId = 'unknown';
    try {
        const [[backup]] = await db.execute('SELECT * FROM server_backups WHERE id = ?', [backupId]);
        if (!backup) {
        ...
        const [existingPanels] = await db.execute('SELECT id FROM reaction_role_panels WHERE guild_id = ?', [guildId]);
        ...
        for (const panel of backupData.reactionRolePanels) {
            const [result] = await db.execute('INSERT INTO reaction_role_panels (guild_id, channel_id, message_id, panel_name, panel_mode) VALUES (?, ?, ?, ?, ?)', [guildId, panel.channel_id, panel.message_id, panel.panel_name, panel.panel_mode]);
...
async function deleteBackup(backupId) {
    let guildId = 'unknown';
    try {
        const [[backup]] = await db.execute('SELECT guild_id FROM server_backups WHERE id = ?', [backupId]);
        if (backup) {
            guildId = backup.guild_id;
        }
