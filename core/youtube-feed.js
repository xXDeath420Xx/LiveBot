...
async function checkYouTubeFeeds() {
    try {
        const [feeds] = await db_1.default.execute('SELECT * FROM youtube_feeds');
        if (feeds.length === 0)
            return;
        ...
        const err = error;
        logger_1.default.error('[YouTubeFeed] Error checking YouTube feeds:', err);
    }
}
    ...
    try {
        ...
        if (latestVideoId && latestVideoId !== feed.last_video_id) {
            ...
            if (!channelName) {
                ...
                await db_1.default.execute('UPDATE youtube_feeds SET channel_name = ? WHERE id = ?', [channelName, feed.id]);
            }
            ...
            await db_1.default.execute('UPDATE youtube_feeds SET last_video_id = ? WHERE id = ?', [latestVideoId, feed.id]);
        }
    }
    catch (error) {
        const err = error;
        logger_1.default.warn(`[YouTubeFeed] Failed to process feed for channel ID ${feed.youtube_channel_id}:`, err.message);
    }
}
