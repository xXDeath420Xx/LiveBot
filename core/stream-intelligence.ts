...
import { getCacheManager } from '../utils/intelligent-cache-manager';
import { RowDataPacket } from 'mysql2';

/**
 * Stream Intelligence System
 *
 * Features:
 * - Adaptive check intervals based on streamer patterns
 * - Viewer count prediction using historical data
 * - Schedule prediction using ML
 * - Stream quality/health monitoring
 * - Anomaly detection (unusual stream times, viewer spikes)
 * - Optimal announcement timing
 * - Streamer behavior clustering
 */
...
class StreamerProfile {
    ...
    constructor(streamerId: string, data: StreamProfileData = {}) {
    ...
    analyzeSchedulePattern(): SchedulePattern {
        ...
        if (avgInterval <= 26) patternType = 'daily';
        else if (avgInterval <= 52) patternType = 'every_other_day';
        else if (avgInterval <= 200) patternType = 'weekly';
    ...
    predictNextStream(): StreamPrediction {
    ...
    getOptimalCheckInterval(): number {
    ...
    predictViewerCount(minutesIntoStream: number): ViewerPrediction {
        if (this.streamHistory.length === 0) {
            return {
    ...
    detectAnomaly(currentData: { viewerCount?: number }): Anomaly[] {
    ...
    updateFromStream(streamData: StreamData): void {
        // Add to history
    ...
    calculateMetrics(): void {
        if (this.streamHistory.length === 0) return;
    ...
    findClosestValue(target: number, array: number[]): number {
        return array.reduce((prev, curr) =>
            Math.abs(curr - target) < Math.abs(prev - target) ? curr : prev
        );
    }

    toJSON(): StreamProfileData {
        return {
...
class StreamIntelligence {
    private profiles: Map<string, StreamerProfile>;
    private cache: any;

    constructor() {
        this.profiles = new Map();
        this.cache = getCacheManager('stream-intelligence', {
            defaultTTL: 3600,
            maxTTL: 7200,
            minTTL: 600
        });
    ...
    async loadProfiles(): Promise<void> {
        try {
            const [rows] = await db.execute<StreamerRow[]>(`
                SELECT
                    s.streamer_id,
                    s.username,
                    s.platform,
            ...
            logger.info(`Loaded ${this.profiles.size} streamer profiles`, { category: 'stream-intelligence' });
        } catch (error) {
            const err = _error as Error;
            logger.error('Failed to load streamer profiles:', {
    ...
    async getProfile(streamerId: string): Promise<StreamerProfile | null> {
        ...
        try {
            ...
            const err = _error as Error;
            logger.error(`Failed to get profile for streamer ${streamerId}:`, {
                category: 'stream-intelligence',
                _error: err.message
            });
            return null;
    ...
    async saveProfile(streamerId: string): Promise<void> {
        ...
        try {
            ...
            logger.debug(`Saved profile for streamer ${streamerId}`, { category: 'stream-intelligence' });
        } catch (error) {
            const err = _error as Error;
            logger.error(`Failed to save profile for streamer ${streamerId}:`, {
                category: 'stream-intelligence',
                _error: err.message
            });
        }
    ...
    async getOptimalCheckInterval(streamerId: string): Promise<number> {
    ...
    async predictNextStream(streamerId: string): Promise<StreamPrediction | null> {
        const profile = await this.getProfile(streamerId);
    ...
    async predictViewerCount(streamerId: string, minutesIntoStream: number): Promise<ViewerPrediction | null> {
        const profile = await this.getProfile(streamerId);
        if (!profile) return null;
    ...
    async detectAnomalies(streamerId: string, currentData: { viewerCount?: number }): Promise<Anomaly[]> {
        const profile = await this.getProfile(streamerId);
        if (!profile) return [];
    ...
    async recordStreamEnd(streamerId: string, streamData: StreamData): Promise<void> {
        const profile = await this.getProfile(streamerId);
        if (!profile) return;
        ...
        logger.info(`Updated profile for streamer ${streamerId} after stream end`, {
            category: 'stream-intelligence',
            metrics: {
    ...
    async getAnalytics(streamerId: string) {
        const profile = await this.getProfile(streamerId);
        if (!profile) return null;
