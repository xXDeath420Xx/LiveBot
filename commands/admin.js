...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addChannelOption(option => option.setName("panel-channel")
        .setDescription("The channel where the request panel will be posted.")
        .addChannelTypes(discord_js_1.ChannelType.GuildText)
        .setRequired(true))
        .addChannelOption(option => option.setName("requests-channel")
        ...
        .addChannelOption(option => option.setName("creator-channel")
        ...
        .setRequired(true))
        .addStringOption(option => option.setName("naming-template")
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'setup-requests') {
            ...
            const requestButton = new discord_js_1.ButtonBuilder()
                .setCustomId(`request_announcement_button_${requestsChannel.id}`)
                .setLabel("Request Announcements")
                .setStyle(discord_js_1.ButtonStyle.Success)
                .setEmoji("📡");
            const row = new discord_js_1.ActionRowBuilder().addComponents(requestButton);
            try {
                ...
                logger_1.default.error("Failed to post request panel:", error);
                await interaction.reply({
            ...
            try {
                if (subcommand === "setup") {
                    ...
                    await db_1.default.execute("INSERT INTO temp_channel_config (guild_id, creator_channel_id, category_id, naming_template) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE creator_channel_id = VALUES(creator_channel_id), category_id = VALUES(category_id), naming_template = VALUES(naming_template)", [interaction.guild.id, creatorChannel.id, category.id, template]);
                    ...
                    await db_1.default.execute("DELETE FROM temp_channel_config WHERE guild_id = ?", [interaction.guild.id]);
                    await interaction.editReply("🗑️ The temporary channel system has been disabled.");
                }
            }
            catch (error) {
                logger_1.default.error("[Temp Channel Command Error]", error);
            ...
            if (subcommand === "setup") {
                ...
                try {
                    await db_1.default.execute(`INSERT INTO join_gate_config (guild_id, verification_enabled, verification_role_id, verification_channel_id)
                         VALUES (?, 1, ?, ?)
                         ON DUPLICATE KEY UPDATE
                            verification_enabled = 1,
                            verification_role_id = VALUES(verification_role_id),
                            verification_channel_id = VALUES(verification_channel_id)`, [interaction.guild.id, role.id, channel.id]);
                    const embed = new discord_js_1.EmbedBuilder()
                        .setColor("#2ECC71")
                    ...
                    logger_1.default.error("[Verification Setup Error]", error);
