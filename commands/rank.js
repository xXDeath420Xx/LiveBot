...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const joinableRanks = interaction.client.joinableRanks;
        if (!joinableRanks) {
        ...
        try {
            if (subcommand === 'join') {
                ...
                const result = await joinableRanks.addRank(interaction.guild.id, role.id, description, category);
                if (result.success) {
                    await interaction.editReply({
                ...
                let updated = false;
                if (description) {
                    const result = await joinableRanks.updateRankDescription(interaction.guild.id, role.id, description);
                    if (result.success)
                        updated = true;
                }
                if (category) {
                    const result = await joinableRanks.updateRankCategory(interaction.guild.id, role.id, category);
                    if (result.success)
                        updated = true;
                }
                if (updated) {
