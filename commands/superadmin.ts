...
async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (subcommand === 'global-reinit') {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                'This action will restart the entire bot application, causing a brief downtime for all guilds.\n\n'
                + '**This will:**\n'
        ...
        const linker = new UserStreamerLinker(interaction.client);
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0x00FF00)
            .addFields(
                { name: 'Guilds Scanned', value: results.totalGuilds.toString(), inline: true },
                { name: 'Members Scanned', value: results.totalMembers.toString(), inline: true },
                { name: 'Streamers Checked', value: results.totalStreamers.toString(), inline: true },
                { name: '✅ Exact Matches', value: results.exactMatches.length.toString(), inline: true },
                { name: '🔍 Fuzzy Matches', value: results.fuzzyMatches.length.toString(), inline: true },
                { name: '🔗 New Links Created', value: results.newLinks.toString(), inline: true },
                { name: '♻️ Existing Links', value: results.existingLinks.toString(), inline: true },
        ...
        if (results.exactMatches.length > 0) {
            const matchList = results.exactMatches.slice(0, 10).map(m =>
                `• ${m.discordUsername} → **${m.streamerUsername}** (${m.platform}) in ${m.guildName}`
            ).join('\n');
        ...
        if (results.fuzzyMatches.length > 0) {
            const fuzzyList = results.fuzzyMatches.slice(0, 5).map(m =>
                `• ${m.discordUsername} ≈ **${m.streamerUsername}** (${m.platform})`
            ).join('\n');
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0x3498DB)
            .addFields(
                { name: 'Matches Found', value: matches.length.toString(), inline: true },
                { name: 'Platform Filter', value: platform || 'All Platforms', inline: true }
            )
            .setTimestamp();

        if (matches.length > 0) {
            const matchList = matches.map(m =>
                `• **${m.discordUsername}** (${m.discordUserId})\n` +
                `  → ${m.streamerUsername} (${m.platform}) in ${m.guildName}`
            ).join('\n\n');
        ...
        if (user) {
            ...
            if (links.length > 0) {
                const linkList = links.map(l =>
                    `• **${l.streamer_username}** (${l.platform})\n` +
                    `  ${l.verified ? '✅ Verified' : '⏳ Pending'} - Linked ${new Date(l.linked_at).toLocaleDateString()}`
                ).join('\n\n');
            ...
            // Show all links
            const [allLinks] = await require('../utils/db').default.query(
            ...
            if (allLinks.length > 0) {
                const linkList = allLinks.slice(0, 20).map((l: any) =>
                    `• <@${l.discord_user_id}> → **${l.streamer_username}** (${l.platform}) ${l.verified ? '✅' : '⏳'}`
                ).join('\n');
                embed.setDescription(linkList);
            } else {
