...
interface TicketConfig extends RowDataPacket {
    guild_id: string;
    support_role_id: string;
}

interface TicketForm extends RowDataPacket {
    form_id: number;
    guild_id: string;
    form_name: string;
}

interface TicketFormQuestion extends RowDataPacket {
    form_id: number;
    question_text: string;
    question_type: 'text' | 'paragraph';
}
...
async function autocomplete(interaction: AutocompleteInteraction): Promise<void> {
    ...
    if (subcommandGroup === 'forms') {
        if (focusedOption.name === "form-name" || focusedOption.name === "name") {
            try {
                const [forms] = await db.execute<TicketForm[]>(
    ...
    if (subcommandGroup === 'forms') {
        ...
        try {
            if (subcommand === "create") {
                ...
                const questionType = interaction.options.getString("question-type", true) as 'text' | 'paragraph';

                const [[form]] = await db.execute<TicketForm[]>(
                ...
                await db.execute(
                    ...
                    [form.form_id, questionText, questionType]
                );
                ...
                for (const form of forms) {
                    const [questions] = await db.execute<TicketFormQuestion[]>(
            ...
            if (error.code === "ER_DUP_ENTRY") {
                ...
                logger.error("[Ticket Forms Command Error]", error as Record<string, any>);
        ...
        try {
            const [[ticket]] = await db.execute<Ticket[]>(
            ...
            const [[config]] = await db.execute<TicketConfig[]>(
            ...
            if (subcommand === "claim") {
                ...
                await db.execute(
                    "UPDATE tickets SET claimed_by_id = NULL WHERE id = ?",
                    [ticket.id]
                );
                ...
                const newStaff = interaction.options.getMember("member") as GuildMember;
            ...
            if (error.code === "ER_NO_SUCH_TABLE" || error.code === "ER_BAD_FIELD_ERROR") {
                ...
                logger.error(`[Ticket Command: ${subcommand}] Error`, error as Record<string, any>);
