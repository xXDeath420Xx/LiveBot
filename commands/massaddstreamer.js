...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
        ...
        if (platform === 'tiktok' || platform === 'youtube') browser = await getBrowser();
        ...
        for (const username of usernames) {
            try {
                ...
                if (platform === 'twitch') { const u = await apiChecks.getTwitchUser(username); if (u) streamerInfo = { puid: u.id, dbUsername: u.login }; }
                else if (platform === 'kick' && cycleTLS) { const u = await apiChecks.getKickUser(cycleTLS, username); if (u) streamerInfo = { puid: u.id.toString(), dbUsername: u.user.username }; } 
                    ...
                    if (channelId) streamerInfo = { puid: channelId, dbUsername: channelName || username }; 
                } 
        ...
        if (cycleTLS) try { cycleTLS.exit(); } catch(e){ console.error('[Mass Add Streamer] Error exiting cycleTLS:', e); } 
