        ...
        if (subcommandGroup === "remind") {
            ...
            try {
                if (subcommand === "me" || subcommand === "in") {
                    ...
                    const [reminders] = await db.execute<ReminderRow[]>(
                        "SELECT id, remind_at, message, is_dm FROM reminders WHERE user_id = ? AND guild_id = ? AND remind_at > NOW() ORDER BY remind_at ASC",
                ...
                logger.error("[Remind Command Error]", error as Record<string, any>);
                await interaction.editReply("An _error occurred while managing your reminders.");
            }
            ...
            try {
                ...
                logger.error("[Welcome Command Error]", error as Record<string, any>);
                await interaction.editReply({ content: "An _error occurred while saving the welcome settings." });
            }
        }
    },
    category: "Events",
