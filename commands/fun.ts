...
interface EconomyConfig extends RowDataPacket {
    guild_id: string;
    enabled: boolean;
    ...
    crime_min: number;
    crime_max: number;
    crime_fail_fine: number;
    rob_min_balance: number;
    rob_success_rate: number;
    rob_fail_fine: number;
    rob_cooldown: number;
        ...
        try {
            ...
            switch (subcommandGroup) {
                case "rep":
                    ...
                    if (subcommand === "give") await reputationHandlers.handleGive(interaction);
                    else if (subcommand === "check") await reputationHandlers.handleCheck(interaction);
                    break;

                case "leaderboard":
                    await interaction.deferReply();
                    if (subcommand === "rep") await leaderboardHandlers.handleRep(interaction);
                    else if (subcommand === "xp") await leaderboardHandlers.handleXp(interaction);
                    break;

                case "giveaway":
                    await interaction.deferReply({ ephemeral: true });
                    if (subcommand === "start") await giveawayHandlers.handleStart(interaction);
                    else if (subcommand === "end") await giveawayHandlers.handleEnd(interaction);
                    else if (subcommand === "reroll") await giveawayHandlers.handleReroll(interaction);
                    else if (subcommand === "list") await giveawayHandlers.handleList(interaction);
                    else if (subcommand === "cancel") await giveawayHandlers.handleCancel(interaction);
                    break;

                case "economy":
                    // Get economy config
                    const [[ecoConfig]] = await db.execute<EconomyConfig[]>(
                    ...
                    await interaction.deferReply();
                    if (subcommand === "balance") await economyHandlers.handleBalance(interaction, ecoConfig);
                    else if (subcommand === "daily") await economyHandlers.handleDaily(interaction, ecoConfig);
                    else if (subcommand === "weekly") await economyHandlers.handleWeekly(interaction, ecoConfig);
                    else if (subcommand === "work") await economyHandlers.handleWork(interaction, ecoConfig);
                    else if (subcommand === "crime") await economyHandlers.handleCrime(interaction, ecoConfig);
                    else if (subcommand === "rob") await economyHandlers.handleRob(interaction, ecoConfig);
                    else if (subcommand === "deposit") await economyHandlers.handleDeposit(interaction, ecoConfig);
                    else if (subcommand === "withdraw") await economyHandlers.handleWithdraw(interaction, ecoConfig);
                    else if (subcommand === "transfer") await economyHandlers.handleTransfer(interaction, ecoConfig);
                    else if (subcommand === "leaderboard") await economyHandlers.handleLeaderboard(interaction, ecoConfig);
                    else if (subcommand === "transactions") await economyHandlers.handleTransactions(interaction, ecoConfig);
                    break;

                case "shop":
                    const [[shopConfig]] = await db.execute<EconomyConfig[]>(
                        "SELECT * FROM economy_config WHERE guild_id = ?",
                        [interaction.guild!.id]
                    );
                    ...
                    if (subcommand === "browse") await shopHandlers.handleBrowse(interaction, shopConfig);
                    else if (subcommand === "buy") await shopHandlers.handleBuy(interaction, shopConfig);
                    else if (subcommand === "sell") await shopHandlers.handleSell(interaction, shopConfig);
                    else if (subcommand === "info") await shopHandlers.handleInfo(interaction, shopConfig);
                    break;

                case "inventory":
                    const [[invConfig]] = await db.execute<EconomyConfig[]>(
                        "SELECT * FROM economy_config WHERE guild_id = ?",
                        [interaction.guild!.id]
                    );
                    ...
                    if (subcommand === "view") await inventoryHandlers.handleView(interaction, invConfig);
                    else if (subcommand === "use") await inventoryHandlers.handleUse(interaction, invConfig);
                    else if (subcommand === "equip") await inventoryHandlers.handleEquip(interaction, invConfig);
                    else if (subcommand === "gift") await inventoryHandlers.handleGift(interaction, invConfig);
                    break;

                case "gamble":
                    const [[gambleConfig]] = await db.execute<EconomyConfig[]>(
                        "SELECT * FROM economy_config WHERE guild_id = ?",
                        [interaction.guild!.id]
                    );
                    ...
                    if (subcommand === "coinflip") await gamblingHandlers.handleCoinflip(interaction, gambleConfig);
                    else if (subcommand === "dice") await gamblingHandlers.handleDice(interaction, gambleConfig);
                    else if (subcommand === "slots") await gamblingHandlers.handleSlots(interaction, gambleConfig);
                    else if (subcommand === "blackjack") await gamblingHandlers.handleBlackjack(interaction, gambleConfig);
                    else if (subcommand === "roulette") await gamblingHandlers.handleRoulette(interaction, gambleConfig);
                    break;

                case "trade":
                    const [[tradeConfig]] = await db.execute<EconomyConfig[]>(
                        "SELECT * FROM economy_config WHERE guild_id = ?",
                        [interaction.guild!.id]
                    );
                    ...
                    if (subcommand === "offer") await tradingHandlers.handleOffer(interaction, tradeConfig);
                    else if (subcommand === "view") await tradingHandlers.handleView(interaction, tradeConfig);
                    else if (subcommand === "accept") await tradingHandlers.handleAccept(interaction, tradeConfig);
                    else if (subcommand === "decline") await tradingHandlers.handleDecline(interaction, tradeConfig);
                    else if (subcommand === "cancel") await tradingHandlers.handleCancel(interaction, tradeConfig);
                    else if (subcommand === "list") await tradingHandlers.handleList(interaction, tradeConfig);
                    break;

                case "trivia":
                    if (subcommand === "start") await triviaHandlers.handleStart(interaction);
                    else if (subcommand === "stats") await triviaHandlers.handleStats(interaction);
                    else if (subcommand === "leaderboard") await triviaHandlers.handleLeaderboard(interaction);
                    break;

                case "hangman":
                    if (subcommand === "start") await hangmanHandlers.handleStart(interaction);
                    else if (subcommand === "guess") await hangmanHandlers.handleGuess(interaction);
                    else if (subcommand === "give-up") await hangmanHandlers.handleGiveUp(interaction);
                    else if (subcommand === "stats") await hangmanHandlers.handleStats(interaction);
                    break;
                ...
                case "counting":
                    ...
                    if (subcommand === "setup") await countingHandlers.handleSetup(interaction);
                    else if (subcommand === "disable") await countingHandlers.handleDisable(interaction);
                    else if (subcommand === "reset") await countingHandlers.handleReset(interaction);
                    else if (subcommand === "stats") await countingHandlers.handleStats(interaction);
                    else if (subcommand === "leaderboard") await countingHandlers.handleLeaderboard(interaction);
                    break;
            ...
            logger.error("[Fun Command Error]", error as Record<string, any>);
