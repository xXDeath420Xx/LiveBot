...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
        ...
        if (newNickname?.toLowerCase() === 'reset') description += `Nickname has been reset to default.\n`;
        else if (newNickname) description += `Nickname set to: **${newNickname}**\n`;
        if (finalAvatarUrl === null) description += `Avatar has been reset to default.\n`;
        else if (finalAvatarUrl !== undefined) description += `Avatar has been updated.\n`;
        
        embed.setDescription(description.trim());
        if (finalAvatarUrl) embed.setThumbnail(finalAvatarUrl);
