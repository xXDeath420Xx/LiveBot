...
const data = new discord_js_1.SlashCommandBuilder()
    ...
    .setRequired(false)
    .addChoices({ name: '⏳ Pending', value: 'pending' }, { name: '👁️ Under Review', value: 'under_review' }, { name: '✅ Approved', value: 'approved' }, { name: '❌ Rejected', value: 'rejected' }, { name: '🚀 Implemented', value: 'implemented' }))
...
async function execute(interaction) {
    ...
    try {
        ...
        logger_1.default.error('[Suggest] Command error:', { error: errorMessage, stack: errorStack });
...
async function handleSubmit(interaction) {
    ...
    // Get config
    const [[config]] = await db_1.default.execute('SELECT * FROM suggestion_config WHERE guild_id = ?', [interaction.guild.id]);
    ...
    try {
        ...
        const [result] = await db_1.default.execute(`INSERT INTO suggestions (guild_id, user_id, suggestion_text, is_anonymous, status)
             VALUES (?, ?, ?, ?, ?)`, [interaction.guild.id, interaction.user.id, suggestion, anonymous, config.require_approval ? 'pending' : 'approved']);
        const suggestionId = result.insertId;
        // Update user stats
        await db_1.default.execute(`INSERT INTO user_suggestion_stats (guild_id, user_id, total_suggestions)
             VALUES (?, ?, 1)
             ON DUPLICATE KEY UPDATE total_suggestions = total_suggestions + 1`, [interaction.guild.id, interaction.user.id]);
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: '👤 Submitted By', value: anonymous ? '👻 Anonymous' : `${interaction.user}`, inline: true }, { name: '📊 Status', value: config.require_approval ? '⏳ Pending Review' : '✅ Approved', inline: true }, { name: '📅 Date', value: new Date().toLocaleDateString(), inline: true })
        ...
        await db_1.default.execute('UPDATE suggestions SET message_id = ?, channel_id = ? WHERE id = ?', [message.id, channel.id, suggestionId]);
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        const errorStack = error instanceof Error ? error.stack : undefined;
        logger_1.default.error('[Suggest] Submit error:', { error: errorMessage, stack: errorStack });
        await interaction.editReply({
...
async function handleView(interaction) {
    ...
    const [[suggestion]] = await db_1.default.execute('SELECT * FROM suggestions WHERE id = ? AND guild_id = ?', [suggestionId, interaction.guild.id]);
    if (!suggestion) {
        await interaction.reply({
    ...
    // Get votes
    const [[voteCounts]] = await db_1.default.execute(`SELECT
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: '👤 Submitted By', value: suggestion.is_anonymous ? '👻 Anonymous' : `<@${suggestion.user_id}>`, inline: true }, { name: '📊 Status', value: getStatusDisplay(suggestion.status), inline: true }, { name: '📅 Date', value: new Date(suggestion.created_at).toLocaleDateString(), inline: true }, { name: '👍 Upvotes', value: `${upvotes}`, inline: true }, { name: '👎 Downvotes', value: `${downvotes}`, inline: true }, { name: '📈 Net Votes', value: `${netVotes}`, inline: true });
...
async function handleList(interaction) {
    ...
    const [suggestions] = await db_1.default.execute(query, params);
...
async function handleApprove(interaction) {
    ...
    // Update status
    await db_1.default.execute(`UPDATE suggestions
         SET status = 'approved', staff_response = ?, reviewed_by = ?, reviewed_at = NOW()
         WHERE id = ?`, [response, interaction.user.id, suggestionId]);
    // Update user stats
    await db_1.default.execute(`UPDATE user_suggestion_stats
         SET approved_suggestions = approved_suggestions + 1,
             reputation_score = reputation_score + 5
         WHERE guild_id = ? AND user_id = ?`, [interaction.guild.id, suggestion.user_id]);
    // Update message if exists
    if (suggestion.message_id && suggestion.channel_id) {
        try {
            const channel = await interaction.guild.channels.fetch(suggestion.channel_id);
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger_1.default.error('[Suggest] Failed to update message:', { error: errorMessage });
        }
    }
    // Notify user
...
async function handleReject(interaction) {
    ...
    await db_1.default.execute(`UPDATE suggestions
         ...
         WHERE id = ?`, [reason, interaction.user.id, suggestionId]);
    // Update user stats
    await db_1.default.execute(`UPDATE user_suggestion_stats
    ...
    if (!suggestion.is_anonymous) {
        try {
            ...
            const dmEmbed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: 'Your Suggestion', value: suggestion.suggestion_text, inline: false }, { name: '💬 Reason', value: reason, inline: false });
            await user.send({ embeds: [dmEmbed] });
        }
        catch (error) {
            // User has DMs disabled
...
async function handleImplement(interaction) {
    // Check permissions
    const member = interaction.member;
    if (!member.permissions.has(discord_js_1.PermissionFlagsBits.ManageGuild)) {
        await interaction.reply({
    ...
    await db_1.default.execute(`UPDATE suggestions
         ...
         WHERE id = ?`, [interaction.user.id, suggestionId]);
    // Update user stats
    await db_1.default.execute(`UPDATE user_suggestion_stats
...
async function handleStats(interaction) {
    ...
    const [[stats]] = await db_1.default.execute('SELECT * FROM user_suggestion_stats WHERE guild_id = ? AND user_id = ?', [interaction.guild.id, targetUser.id]);
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: '📝 Total Suggestions', value: `${stats.total_suggestions}`, inline: true }, { name: '✅ Approved', value: `${stats.approved_suggestions}`, inline: true }, { name: '❌ Rejected', value: `${stats.rejected_suggestions}`, inline: true }, { name: '🚀 Implemented', value: `${stats.implemented_suggestions}`, inline: true }, { name: '📊 Approval Rate', value: `${approvalRate}%`, inline: true }, { name: '⭐ Reputation', value: `${stats.reputation_score}`, inline: true }, { name: '👍 Total Upvotes', value: `${stats.total_upvotes_received}`, inline: true }, { name: '👎 Total Downvotes', value: `${stats.total_downvotes_received}`, inline: true })
        .setTimestamp();
    await interaction.reply({ embeds: [embed] });
}
async function handleSetup(interaction) {
    ...
    await db_1.default.execute(`INSERT INTO suggestion_config (guild_id, suggestions_channel_id, enabled)
         VALUES (?, ?, TRUE)
         ON DUPLICATE KEY UPDATE suggestions_channel_id = ?, enabled = TRUE`, [interaction.guild.id, channel.id, channel.id]);
    const embed = new discord_js_1.EmbedBuilder()
        .setColor('#00FF00')
        .setTitle('✅ Suggestions System Configured')
        .setDescription(`Suggestions system has been set up successfully!`)
        .addFields({ name: '📍 Suggestions Channel', value: `${channel}`, inline: false }, { name: '📋 Commands', value: '`/suggest submit` - Submit a suggestion\n`/suggest view <id>` - View a suggestion\n`/suggest list` - List suggestions\n`/suggest approve <id>` - Approve (Staff)\n`/suggest reject <id>` - Reject (Staff)', inline: false })
        .setTimestamp();
    await interaction.reply({ embeds: [embed] });
}
...
async function handleButton(interaction) {
    if (!interaction.customId.startsWith('suggest_'))
        return false;
    const [action, voteType, suggestionIdStr] = interaction.customId.split('_');
    const suggestionId = parseInt(suggestionIdStr);
    if (action !== 'suggest')
        return false;
    // Get current vote
    const [[existingVote]] = await db_1.default.execute('SELECT vote_type FROM suggestion_votes WHERE suggestion_id = ? AND user_id = ?', [suggestionId, interaction.user.id]);
    try {
        if (existingVote && existingVote.vote_type === voteType) {
            // Remove vote
            await db_1.default.execute('DELETE FROM suggestion_votes WHERE suggestion_id = ? AND user_id = ?', [suggestionId, interaction.user.id]);
            await interaction.reply({
            ...
            // Add or update vote
            await db_1.default.execute(`INSERT INTO suggestion_votes (suggestion_id, user_id, vote_type)
                 VALUES (?, ?, ?)
                 ON DUPLICATE KEY UPDATE vote_type = ?`, [suggestionId, interaction.user.id, voteType, voteType]);
            await interaction.reply({
        ...
        const upvotes = voteCounts?.upvotes || 0;
        const downvotes = voteCounts?.downvotes || 0;
        await db_1.default.execute('UPDATE suggestions SET upvotes = ?, downvotes = ? WHERE id = ?', [upvotes, downvotes, suggestionId]);
        ...
        // Update user stats
        const [[suggestion]] = await db_1.default.execute('SELECT user_id FROM suggestions WHERE id = ?', [suggestionId]);
        if (suggestion) {
            await db_1.default.execute(`UPDATE user_suggestion_stats
                 SET total_upvotes_received = ?,
                     total_downvotes_received = ?
                 WHERE guild_id = ? AND user_id = ?`, [upvotes, downvotes, interaction.guild.id, suggestion.user_id]);
        }
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger_1.default.error('[Suggest] Vote error:', { error: errorMessage });
        await interaction.reply({
