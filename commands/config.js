...
function verifyPassword(plainPassword, hash) {
    ...
    const derivedKey = crypto_1.default.scryptSync(plainPassword, salt, 64);
...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addChannelOption(o => o.setName("channel").setDescription("The channel for notifications").addChannelTypes(discord_js_1.ChannelType.GuildText, discord_js_1.ChannelType.GuildAnnouncement).setRequired(true)))
        .addSubcommand(subcommand => subcommand
        .setName('setliverole')
        ...
        .setRequired(true)
        .addChoices({ name: "Twitch", value: "twitch" }, { name: "Kick", value: "kick" }, { name: "YouTube", value: "youtube" }, { name: "tiktok", value: "tiktok" }, { name: "Trovo", value: "trovo" }))
    ...
    async autocomplete(interaction) {
        ...
        if (subcommand === 'customizestreamer' && focusedOption.name === "username") {
            ...
            try {
                const [streamers] = await db_1.default.execute("SELECT username FROM streamers WHERE guild_id = ? AND platform = ? AND username LIKE ? LIMIT 25", [interaction.guild.id, platform, `${focusedValue}%`]);
    ...
    async execute(interaction) {
        ...
        switch (subcommand) {
            case 'setchannel': {
                ...
                try {
                    await db_1.default.execute("INSERT INTO guilds (guild_id, announcement_channel_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE announcement_channel_id = VALUES(announcement_channel_id)", [guildId, channel.id]);
                    ...
                    logger_1.default.error("[SetChannel Error]", e);
                ...
                try {
                    if (role) {
                        ...
                        if (role.position >= botMember.roles.highest.position) {
                            await interaction.editReply({ content: `Error: The "${role.name}" role is higher than my role in the server hierarchy, so I cannot assign it.` });
                            return;
                        }
                    }
                    await db_1.default.execute("INSERT INTO guilds (guild_id, live_role_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE live_role_id = VALUES(live_role_id)", [guildId, roleId]);
                    const embed = new discord_js_1.EmbedBuilder().setColor(role ? "#57F287" : "#ED4245").setTitle("Live Role Updated");
                    embed.setDescription(role ? `The live role has been set to ${role}.` : "The live role has been cleared and is now disabled.");
                    await interaction.editReply({ embeds: [embed] });
                }
                catch (e) {
                    logger_1.default.error("[SetLiveRole Error]", e);
            ...
            case 'set-dj-role': {
                ...
                const guildId = interaction.guild.id;
                try {
                    await db_1.default.execute("INSERT INTO music_config (guild_id, dj_role_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE dj_role_id = ?", [guildId, role.id, role.id]);
                    await interaction.reply({ content: `✅ The DJ role has been set to <@&${role.id}>.`, flags: [discord_js_1.MessageFlagsBitField.Flags.Ephemeral] });
                }
                catch (error) {
                    logger_1.default.error("[Config DJ Role Error]", error);
                    await interaction.reply({ content: "❌ An error occurred while setting the DJ role.", flags: [discord_js_1.MessageFlagsBitField.Flags.Ephemeral] });
                }
                break;
            }
            case 'logging': {
                ...
                try {
                    await db_1.default.execute("INSERT INTO log_config (guild_id, log_channel_id, enabled_logs) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE log_channel_id = VALUES(log_channel_id), enabled_logs = VALUES(enabled_logs)", [interaction.guild.id, channel.id, JSON.stringify(enabledLogs)]);
                    const embed = new discord_js_1.EmbedBuilder()
                        .setColor("#57F287")
                    ...
                    logger_1.default.error("[Logging Command Error]", error);
                ...
                try {
                    ...
                    if (shouldResetNickname) {
                        try {
                            ...
                            await db_1.default.execute("UPDATE guilds SET bot_nickname = NULL WHERE guild_id = ?", [guildId]);
                            nicknameUpdated = true;
                        }
                        catch (e) {
                            logger_1.default.error("[Customize Bot Command] Failed to reset nickname:", e);
                            await interaction.editReply({ content: "Failed to reset nickname. My role is likely not high enough in the role list or I lack permissions." });
                            return;
                        }
                    }
                    else if (newNickname) {
                        try {
                            ...
                            await db_1.default.execute("UPDATE guilds SET bot_nickname = ? WHERE guild_id = ?", [newNickname, guildId]);
                            nicknameUpdated = true;
                        }
                        catch (e) {
                            logger_1.default.error("[Customize Bot Command] Failed to set nickname:", e);
                            await interaction.editReply({ content: "Failed to set nickname. My role is likely not high enough in the role list." });
                            return;
                        }
                    }
                    ...
                    if (resetAvatarFlag) {
                        ...
                        if (!newAvatar.contentType?.startsWith("image/")) {
                            await interaction.editReply({ content: "Avatar must be an image file (PNG, JPG, GIF)." });
                            return;
                        }
                        ...
                        try {
                            ...
                            logger_1.default.error("[Customize Bot Command] Error uploading temporary avatar to Discord:", uploadError);
                    ...
                    if (permanentAvatarUrl !== undefined) {
                        await db_1.default.execute("UPDATE guilds SET webhook_avatar_url = ? WHERE guild_id = ?", [permanentAvatarUrl, guildId]);
                        finalAvatarUrlForEmbed = permanentAvatarUrl;
                    }
                    if (!nicknameUpdated && !avatarUpdated && !avatarReset) {
                        await interaction.editReply({ content: "No changes were requested." });
                        return;
                    }
                    const embed = new discord_js_1.EmbedBuilder().setColor("#57F287").setTitle("Bot Appearance Updated!");
                    let description = "";
                    if (shouldResetNickname) {
                    ...
                    logger_1.default.error('Customize Bot Error:', error);
            ...
            case 'customizechannel': {
                ...
                if (newNickname === null && newAvatarAttachment === null && newAvatarUrlText === null) {
                    await interaction.editReply("You must provide a nickname, an avatar file, or an avatar URL to set/reset.");
                    return;
                }
                ...
                try {
                    if (newAvatarAttachment) {
                        ...
                        const tempUploadChannelId = process.env.TEMP_UPLOAD_CHANNEL_ID;
                        if (!tempUploadChannelId) {
                            logger_1.default.error("[Customize Channel Command] TEMP_UPLOAD_CHANNEL_ID is not configured.");
                            await interaction.editReply({ content: "Temporary upload channel ID is not configured. Please set TEMP_UPLOAD_CHANNEL_ID in your .env file." });
                            return;
                        }
                        try {
                            ...
                            logger_1.default.error("[Customize Channel Command] Error uploading temporary avatar to Discord:", uploadError);
                            await interaction.editReply({ content: "Failed to upload custom avatar. Please check bot's permissions or TEMP_UPLOAD_CHANNEL_ID." });
                            return;
                        }
                    }
                        ...
                        if (newAvatarUrlText?.toLowerCase() === "reset" || newAvatarUrlText === "") {
                            ...
                            if (!newAvatarUrlText.startsWith('http://') && !newAvatarUrlText.startsWith('https://')) {
                                await interaction.editReply("The provided avatar URL must start with `http://` or `https://`.");
                                return;
                            }
                    ...
                    await db_1.default.execute(`INSERT INTO channel_settings (${insertColumns.join(", ")}) VALUES (${insertPlaceholders.join(", ")}) ON DUPLICATE KEY UPDATE ${updateClauses.join(", ")}`, [...insertValues, ...updateValuesForDuplicateKey]);
                    const embed = new discord_js_1.EmbedBuilder().setColor("#57F287").setTitle(`Channel Customization Updated for #${channel.name}`);
                    let description = "";
                    ...
                    logger_1.default.error("[Customize Channel Error]", error);
            ...
            case 'customizestreamer': {
                ...
                if (newNickname === null && newAvatarAttachment === null && newAvatarUrlText === null && newMessage === null) {
                    await interaction.editReply("You must provide at least one item to customize (nickname, avatar, or message).");
                    return;
                }
                let finalAvatarUrl = undefined;
                try {
                    if (newAvatarUrlText !== null) {
                        ...
                        const tempUploadChannelId = process.env.TEMP_UPLOAD_CHANNEL_ID;
                        if (!tempUploadChannelId) {
                            logger_1.default.error("[Customize Streamer Command] TEMP_UPLOAD_CHANNEL_ID is not configured.");
                            await interaction.editReply({ content: "Temporary upload channel ID is not configured. Please set TEMP_UPLOAD_CHANNEL_ID in your .env file." });
                            return;
                        }
                        try {
                            ...
                            logger_1.default.error("[Customize Streamer Command] Error uploading temporary avatar to Discord:", uploadError);
                            await interaction.editReply({ content: "Failed to upload custom avatar. Please check bot's permissions or TEMP_UPLOAD_CHANNEL_ID." });
                            return;
                        }
                    }
                    const [[streamer]] = await db_1.default.execute("SELECT s.streamer_id FROM streamers s WHERE s.platform = ? AND s.username = ?", [platform, username]);
                    if (!streamer) {
                        await interaction.editReply(`Streamer \`${username}\` (${platform}) was not found.`);
                        return;
                    }
                    const [[subscription]] = await db_1.default.execute("SELECT subscription_id FROM subscriptions WHERE guild_id = ? AND streamer_id = ? AND announcement_channel_id <=> ?", [interaction.guild.id, streamer.streamer_id, targetChannelId]);
                    if (!subscription) {
                        const channelName = channel ? `in ${channel}` : "in the server default channel";
                        await interaction.editReply(`That streamer is not configured to announce ${channelName}.`);
                        return;
                    }
                    const updates = [];
                    const values = [];
                    ...
                    await db_1.default.execute(`UPDATE subscriptions SET ${updates.join(", ")} WHERE guild_id = ? AND streamer_id = ? AND announcement_channel_id <=> ?`, values);
                    const embed = new discord_js_1.EmbedBuilder()
                        .setColor("#57F287")
                    ...
                    logger_1.default.error("[Customize Streamer Error]", error);
                    await interaction.editReply(`An error occurred while updating the streamer customization: ${error.message}`);
                }
                break;
            }
            default:
                await interaction.reply({ content: 'Invalid config subcommand.', flags: [discord_js_1.MessageFlagsBitField.Flags.Ephemeral] });
                break;
        }
    },
};
