        ...
        if (subcommand === 'customizestreamer' && focusedOption.name === "username") {
            ...
            try {
                const [streamers] = await db.execute<StreamerRow[]>(
                    "SELECT username FROM streamers WHERE guild_id = ? AND platform = ? AND username LIKE ? LIMIT 25",
                    [interaction.guild!.id, platform, `${focusedValue}%`]
                );
        ...
        switch (subcommand) {
            case 'setchannel': {
                ...
                try {
                    ...
                    logger.error("[SetChannel Error]", e as Record<string, any>);
            ...
            case 'setliverole': {
                ...
                try {
                    ...
                    logger.error("[SetLiveRole Error]", e as Record<string, any>);
            ...
            case 'set-dj-role': {
                ...
                try {
                    await db.execute(
                        "INSERT INTO music_config (guild_id, dj_role_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE dj_role_id = ?",
                    ...
                    logger.error("[Config DJ Role Error]", error as Record<string, any>);
            ...
            case 'logging': {
                ...
                try {
                    ...
                    logger.error("[Logging Command Error]", error as Record<string, any>);
                    await interaction.editReply({ content: "An _error occurred while saving logging settings." });
                }
                break;
            }
            case 'customizebot': {
                ...
                let finalAvatarUrlForEmbed: string | null = null;
                ...
                try {
                    ...
                    if (shouldResetNickname) {
                        try {
                            ...
                            logger.error("[Customize Bot Command] Failed to reset nickname:", e as Record<string, any>);
                        ...
                        try {
                            ...
                            logger.error("[Customize Bot Command] Failed to set nickname:", e as Record<string, any>);
                            await interaction.editReply({ content: "Failed to set nickname. My role is likely not high enough in the role list." });
                            return;
                        }
                    }

                    let permanentAvatarUrl: string | null | undefined = undefined;
                    if (resetAvatarFlag) {
                        ...
                        try {
                            const tempChannel = await interaction.client.channels.fetch(tempUploadChannelId) as TextChannel;
                            if (!tempChannel) {
                            ...
                            logger.error("[Customize Bot Command] Error uploading temporary avatar to Discord:", uploadError as Record<string, any>);
                    ...
                    logger.error('Customize Bot Error:', error as Record<string, any>);
                ...
                try {
                    if (newAvatarAttachment) {
                        ...
                        try {
                            ...
                            logger.error("[Customize Channel Command] Error uploading temporary avatar to Discord:", uploadError as Record<string, any>);
                            await interaction.editReply({ content: "Failed to upload custom avatar. Please check bot's permissions or TEMP_UPLOAD_CHANNEL_ID." });
                            return;
                        }
                    ...
                    const insertValues: any[] = [channel.id, interaction.guild!.id];
                    const updateClauses: string[] = [];
                    const updateValuesForDuplicateKey: any[] = [];
                    ...
                    logger.error("[Customize Channel Error]", error as Record<string, any>);
                ...
                try {
                    if (newAvatarUrlText !== null) {
                        ...
                        try {
                            ...
                            logger.error("[Customize Streamer Command] Error uploading temporary avatar to Discord:", uploadError as Record<string, any>);
                            await interaction.editReply({ content: "Failed to upload custom avatar. Please check bot's permissions or TEMP_UPLOAD_CHANNEL_ID." });
                            return;
                        }
                    }
                    ...
                    const [[subscription]] = await db.execute<SubscriptionRow[]>(
                    ...
                    logger.error("[Customize Streamer Error]", error as Record<string, any>);
