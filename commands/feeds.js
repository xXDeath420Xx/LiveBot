  ...
  async autocomplete(interaction) {
    ...
    if (subcommandGroup === 'reddit' && focusedOption.name === 'subreddit') {
        ...
        try {
            ...
            logger.error("[TikTok Command Autocomplete Error]", error);
            await interaction.respond([]);
        }
    } else if (subcommandGroup === 'youtube' && focusedOption.name === 'youtube_channel_id') {
        const focusedValue = focusedOption.value;
        try {
            const [subscriptions] = await db.execute("SELECT youtube_channel_id, channel_name FROM youtube_subscriptions WHERE guild_id = ? AND channel_name LIKE ?", [interaction.guild.id, `${focusedValue}%`]);
            await interaction.respond(subscriptions.map(sub => ({name: `${sub.channel_name} (${sub.youtube_channel_id})`, value: sub.youtube_channel_id})));
        } catch (error) {
            logger.error("[YouTube Command Autocomplete Error]", error);
            await interaction.respond([]);
        }
  ...
  async execute(interaction) {
    ...
    if (subcommandGroup === 'reddit') {
        ...
        try {
            ...
            logger.error("[TikTok Command Error]", error);
        ...
        try {
            if (subcommand === "add") {
                ...
                const [result] = await db.execute("DELETE FROM youtube_subscriptions WHERE guild_id = ? AND youtube_channel_id = ?", [guildId, youtubeChannelId]);
