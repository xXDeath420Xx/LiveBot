    ...
    start(botClient, statusGetter) {
        ...
        app.post('/manage/:guildId/add', checkAuth, checkGuildAdmin, async (req, res) => {
            ...
            try {
                ...
                if (existingStreamer) {
                    ...
                    await db.execute(
                        `UPDATE streamers SET username=?, platform_user_id=? WHERE streamer_id=?`,
        ...
        app.post('/manage/:guildId/edit-subscription', checkAuth, checkGuildAdmin, async (req, res) => {
            ...
            try {
                ...
                await db.execute(
                    `UPDATE subscriptions SET announcement_channel_id=?, custom_message=?, override_nickname=?, override_avatar_url=? WHERE subscription_id=?`,
            ...
            for (const teamSub of teamSubscriptionsRaw) {
                ...
                if (channelsData[channelId]) { 
                    ...
                    const [teamMembers] = await db.execute(
                        `SELECT s.streamer_id, s.username, s.platform, s.profile_image_url FROM subscriptions sub JOIN streamers s ON sub.streamer_id = s.streamer_id WHERE sub.guild_id = ? AND sub.announcement_channel_id = ? AND (s.platform = 'twitch' OR s.platform = 'kick')`,
