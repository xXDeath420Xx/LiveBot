...
function start(botClient) {
    ...
    const checkGuildAdmin = (req, res, next) => {
        try {
            ...
            if (!req.user || !req.user.guilds) {
                if (isApiRequest) return res.status(403).json({ error: true, message: 'Authentication error' });
            ...
            if (isApiRequest) return res.status(403).json({ error: true, message: 'Permission denied.' });
    ...
    app.get('/manage/:guildId', checkAuth, checkGuildAdmin, async (req, res) => {
        try {
            ...
            for (const teamSub of rawTeamSubscriptions) {
                const [rawMembers] = await db.execute(`SELECT sub.*, s.platform, s.username, s.kick_username, s.discord_user_id FROM subscriptions sub JOIN streamers s ON sub.streamer_id = s.streamer_id WHERE sub.guild_id = ? AND sub.announcement_channel_id = ? AND s.platform = 'twitch'`, [guildId, teamSub.announcement_channel_id]);
                const members = rawMembers.map(rawMember => {
                    ...
                    if (rawMember.discord_user_id) {
                        ...
                        if(linked?.kick) { const kickData = streamerIdToDataMap.get(linked.kick); if(kickData) kickUsername = kickData.username; }
                    ...
                    if(kickUsername){
                        const kickStreamer = allStreamers.find(s => s.platform === 'kick' && s.username === kickUsername);
                        if(kickStreamer) processedStreamerSubscriptionKeys.add(`${kickStreamer.streamer_id}-${teamSub.announcement_channel_id}`);
                    }
    ...
    app.post('/manage/:guildId/add', checkAuth, checkGuildAdmin, async (req, res) => {
        ...
        try {
            if (platform === 'twitch') { const u = await apiChecks.getTwitchUser(username); if(u) streamerInfo = { puid: u.id, dbUsername: u.login, pfp: u.profile_image_url }; }
            ...
            if (!streamer || !streamer.streamer_id) return res.status(500).render('error', { user: req.user, error: 'Failed to retrieve streamer ID.' });
        ...
        } catch (error) { console.error('[Dashboard Add Streamer Error]:', error); res.status(500).render('error', { user: req.user, error: 'An error occurred.' }); }
    });
    ...
    app.post('/manage/:guildId/edit-subscription', checkAuth, checkGuildAdmin, async (req, res) => {
        ...
        if (!sub) return res.status(404).render('error', { user: req.user, error: 'Subscription not found.' });
    ...
    app.get('/manage/:guildId/export', checkAuth, checkGuildAdmin, async (req, res) => {
        const [rows] = await db.execute(`SELECT s.platform, s.username, s.discord_user_id, s.kick_username, sub.announcement_channel_id, sub.custom_message FROM subscriptions sub JOIN streamers s ON sub.streamer_id = s.streamer_id WHERE sub.guild_id = ?`, [req.params.guildId]);
        if (rows.length === 0) return res.status(404).send('No subscriptions to export.');
    ...
    app.post('/manage/:guildId/import-team', checkAuth, checkGuildAdmin, upload.single('csvfile'), async (req, res) => {
        if (!req.file) return res.status(400).render('error', { user: req.user, error: 'No CSV file uploaded.' });
        const csvData = fs.readFileSync(req.file.path, 'utf8');
        fs.unlinkSync(req.file.path);
        const { data: rows } = Papa.parse(csvData, { header: true, skipEmptyLines: true });

        for (const row of rows) {
            if (!row.platform || !row.username) continue;
    ...
    async function getFormattedLiveRows(rows) {
        ...
        for (const row of rows) {
            ...
            if (!streamersMap.has(key)) streamersMap.set(key, []);
        ...
        if (discordUserIds.length > 0) {
            ...
            for (const acc of accounts) {
                if (!allAssociatedAccounts.has(acc.discord_user_id)) allAssociatedAccounts.set(acc.discord_user_id, []);
        ...
        for (const userAnnouncements of streamersMap.values()) {
            ...
            if (!primaryLiveAnnouncement.username) continue;
            ...
            const live_platforms = [...new Map(userAnnouncements.map(a => {
                let url = '#';
                switch(a.platform){
                    case 'twitch': url = `https://www.twitch.tv/${a.username}`; break;
                    case 'kick': url = `https://kick.com/${a.username}`; break;
                    case 'youtube': url = `https://www.youtube.com/channel/${a.platform_user_id}`; break;
                    case 'tiktok': url = `https://www.tiktok.com/@${a.username}`; break;
                    case 'trovo': url = `https://trovo.live/s/${a.username}`; break;
                }
    ...
    app.get('/api/status-data', async (req, res) => {
        try {
            ...
            const [
                ...
                db.execute(`SELECT s.username, s.discord_user_id, s.profile_image_url, s.platform_user_id, s.streamer_id, a.platform, a.stream_game FROM announcements a JOIN streamers s ON a.streamer_id = s.streamer_id WHERE s.username IS NOT NULL AND s.username != '' AND a.platform IS NOT NULL AND a.platform != ''`)
            ]);
            ...
            if (req.isAuthenticated()) {
                const pm2DataPromise = new Promise((resolve) => {
                    pm2.describe('CertiFriedAnnouncer', (err, procList) => {
                        if (err || !procList || procList.length === 0) return resolve({ app: { status: 'offline', uptime: 'N/A', memory: 'N/A' } });
    ...
    app.get('/api/authenticated-logs', checkAuth, async (req, res) => {
        ...
        exec(logCommand, { maxBuffer: 1024 * 500 }, (err, stdout, stderr) => {
            if (err) return res.status(500).json({ logs: `Log file not found at: ${logPath}`, error: 'Failed to read logs.' });
            res.json({ logs: stdout, error: stderr || null });
        });
    });

    app.get('/api/global-live-status', async (req, res) => {
        const [liveRows] = await db.execute(`SELECT s.username, s.discord_user_id, s.profile_image_url, s.platform_user_id, a.platform, a.stream_game FROM announcements a JOIN streamers s ON a.streamer_id = s.streamer_id WHERE s.username IS NOT NULL AND s.username != ''`);
    ...
    app.get('/api/guilds/:guildId/livestatus', checkAuth, checkGuildAdmin, async (req, res) => {
        const [liveRows] = await db.execute(`SELECT s.username, s.discord_user_id, s.profile_image_url, s.platform_user_id, a.platform, a.stream_game FROM announcements a JOIN streamers s ON a.streamer_id = s.streamer_id WHERE a.guild_id = ? AND s.username IS NOT NULL AND s.username != ''`, [req.params.guildId]);
