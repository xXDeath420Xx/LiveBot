...
async function handleBrowse(interaction, config) {
...
async function handleBuy(interaction, config) {
    ...
    // Get item details
    const [[item]] = await db_1.default.execute(`SELECT * FROM shop_items
         WHERE id = ? AND (guild_id = ? OR guild_id IS NULL) AND is_active = TRUE`, [itemId, interaction.guild.id]);
    if (!item) {
    ...
    if (item.required_level > 0) {
        const [[userLevel]] = await db_1.default.execute("SELECT level FROM user_levels WHERE guild_id = ? AND user_id = ?", [interaction.guild.id, interaction.user.id]);
    ...
    if (item.required_item_id) {
        const [[hasRequired]] = await db_1.default.execute("SELECT id FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, interaction.user.id, item.required_item_id]);
        if (!hasRequired) {
            const [[requiredItem]] = await db_1.default.execute("SELECT item_name FROM shop_items WHERE id = ?", [item.required_item_id]);
            return interaction.editReply({
    ...
    if (item.max_per_user !== -1) {
        const [[userInventory]] = await db_1.default.execute("SELECT COALESCE(quantity, 0) as quantity FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, interaction.user.id, itemId]);
    ...
    // Get user economy
    const [[userEconomy]] = await db_1.default.execute("SELECT * FROM user_economy WHERE guild_id = ? AND user_id = ?", [interaction.guild.id, interaction.user.id]);
    if (!userEconomy) {
        return interaction.editReply({
    ...
    try {
        ...
        // Deduct money
        await db_1.default.execute("UPDATE user_economy SET wallet = wallet - ?, total_spent = total_spent + ? WHERE guild_id = ? AND user_id = ?", [totalCost, totalCost, interaction.guild.id, interaction.user.id]);
        // Add item to inventory
        const [[existingItem]] = await db_1.default.execute("SELECT * FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, interaction.user.id, itemId]);
        if (existingItem) {
            ...
            await db_1.default.execute("INSERT INTO user_inventory (guild_id, user_id, item_id, quantity) VALUES (?, ?, ?, ?)", [interaction.guild.id, interaction.user.id, itemId, quantity]);
        }
        // Update stock if not unlimited
        if (item.stock !== -1) {
            await db_1.default.execute("UPDATE shop_items SET stock = stock - ? WHERE id = ?", [quantity, itemId]);
        }
        // Log transaction
        ...
        if (item.item_type === "role" && item.role_id) {
            try {
                ...
                logger_1.default.error("[Shop] Failed to grant role:", { error: error.message });
            }
        }
        const embed = new discord_js_1.EmbedBuilder()
            .setColor("#00FF00")
            ...
            .addFields({ name: "Total Cost", value: formatMoney(totalCost, config), inline: true }, { name: "Remaining Balance", value: formatMoney(userEconomy.wallet - totalCost, config), inline: true })
            .setTimestamp();
        ...
        logger_1.default.error("[Shop] Purchase error:", { error: error.message, stack: error.stack });
...
async function handleSell(interaction, config) {
    ...
    const quantity = interaction.options.getInteger("quantity") || 1;
    // Get item details
    const [[item]] = await db_1.default.execute("SELECT * FROM shop_items WHERE id = ?", [itemId]);
    if (!item) {
        return interaction.editReply({
    ...
    const [[userInventory]] = await db_1.default.execute("SELECT * FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, interaction.user.id, itemId]);
    ...
    try {
        await db_1.default.execute("START TRANSACTION");
        // Add money
        await db_1.default.execute("UPDATE user_economy SET wallet = wallet + ?, total_earned = total_earned + ? WHERE guild_id = ? AND user_id = ?", [totalValue, totalValue, interaction.guild.id, interaction.user.id]);
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: "Total Value", value: formatMoney(totalValue, config), inline: true }, { name: "New Balance", value: formatMoney(userEconomy.wallet + totalValue, config), inline: true })
            .setTimestamp();
        await interaction.editReply({ embeds: [embed] });
    }
    catch (error) {
        await db_1.default.execute("ROLLBACK");
        logger_1.default.error("[Shop] Sell error:", { error: error.message, stack: error.stack });
        await interaction.editReply({
...
async function handleInfo(interaction, config) {
    ...
    const [[item]] = await db_1.default.execute(`SELECT s.*,
                COALESCE((SELECT quantity FROM user_inventory WHERE item_id = s.id AND guild_id = ? AND user_id = ?), 0) as user_owned,
                COALESCE((SELECT SUM(quantity) FROM user_inventory WHERE item_id = s.id AND guild_id = ?), 0) as total_owned
         FROM shop_items s
         WHERE s.id = ? AND (s.guild_id = ? OR s.guild_id IS NULL)`, [interaction.guild.id, interaction.user.id, interaction.guild.id, itemId, interaction.guild.id]);
    if (!item) {
        return interaction.reply({
            content: "❌ Item not found.",
            ephemeral: true
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "💰 Price", value: formatMoney(item.price, config), inline: true }, { name: "📦 Type", value: getCategoryName(item.item_type), inline: true }, { name: "📊 Stock", value: item.stock === -1 ? "Unlimited" : `${item.stock} remaining`, inline: true });
