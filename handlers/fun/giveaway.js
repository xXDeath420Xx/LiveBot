...
async function handleStart(interaction) {
    ...
    await db_1.default.execute("INSERT INTO giveaways (guild_id, channel_id, message_id, prize, winner_count, ends_at, host_id) VALUES (?, ?, ?, ?, ?, ?, ?)", [guildId, interaction.channel.id, giveawayMessage.id, prize, winners, endsAt, interaction.user.id]);
    await interaction.editReply({ content: "✅ Giveaway started!" });
}
async function handleEnd(interaction) {
    const messageId = interaction.options.getString("message-id");
    const guildId = interaction.guild.id;
    const [[giveaway]] = await db_1.default.execute("SELECT * FROM giveaways WHERE message_id = ? AND guild_id = ? AND is_active = 1", [messageId, guildId]);
    if (!giveaway) {
...
async function handleReroll(interaction) {
    const messageId = interaction.options.getString("message-id");
    const guildId = interaction.guild.id;
    const [[giveaway]] = await db_1.default.execute("SELECT * FROM giveaways WHERE message_id = ? AND guild_id = ?", [messageId, guildId]);
    if (!giveaway) {
...
async function handleList(interaction) {
    const guildId = interaction.guild.id;
    const [giveaways] = await db_1.default.execute("SELECT * FROM giveaways WHERE guild_id = ? AND is_active = 1 ORDER BY ends_at ASC", [guildId]);
...
async function handleCancel(interaction) {
    ...
    await db_1.default.execute("UPDATE giveaways SET is_active = 0 WHERE id = ?", [giveaway.id]);
    const channel = await interaction.guild.channels.fetch(giveaway.channel_id).catch(() => null);
    if (channel) {
        const message = await channel.messages.fetch(giveaway.message_id).catch(() => null);
        if (message) {
            const cancelledEmbed = discord_js_1.EmbedBuilder.from(message.embeds[0])
                .setColor("#E74C3C")
