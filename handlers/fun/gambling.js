...
exports.handleRoulette = handleRoulette;
...
async function handleCoinflip(interaction, config) {
    ...
    // Update balance
    await db_1.default.execute("UPDATE user_economy SET wallet = wallet + ?, total_earned = total_earned + ? WHERE guild_id = ? AND user_id = ?", [netChange, won ? betAmount : 0, interaction.guild.id, interaction.user.id]);
    // Log transaction
    await db_1.default.execute(`INSERT INTO gambling_history (guild_id, user_id, game_type, bet_amount, win_amount, result)
         VALUES (?, ?, 'coinflip', ?, ?, ?)`, [interaction.guild.id, interaction.user.id, betAmount, winAmount, result]);
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "Bet Amount", value: formatMoney(betAmount, config), inline: true }, { name: "Result", value: won ? `✅ Won ${formatMoney(winAmount, config)}` : `❌ Lost ${formatMoney(betAmount, config)}`, inline: true }, { name: "New Balance", value: formatMoney(userEconomy.wallet + netChange, config), inline: true })
        .setTimestamp();
...
async function handleDice(interaction, config) {
    ...
    // Update balance
    await db_1.default.execute("UPDATE user_economy SET wallet = wallet + ?, total_earned = total_earned + ? WHERE guild_id = ? AND user_id = ?", [netChange, won ? winAmount : 0, interaction.guild.id, interaction.user.id]);
    // Log transaction
    await db_1.default.execute(`INSERT INTO gambling_history (guild_id, user_id, game_type, bet_amount, win_amount, result)
         VALUES (?, ?, 'dice', ?, ?, ?)`, [interaction.guild.id, interaction.user.id, betAmount, winAmount, `${dice1}+${dice2}=${total}`]);
    await db_1.default.execute(`INSERT INTO economy_transactions (guild_id, user_id, transaction_type, amount, balance_before, balance_after, description)
         VALUES (?, ?, 'gamble', ?, ?, ?, ?)`, [
        interaction.guild.id,
        interaction.user.id,
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "Bet Amount", value: formatMoney(betAmount, config), inline: true }, { name: "Result", value: won ? `✅ Won ${formatMoney(winAmount, config)} (${multiplier}x)` : `❌ Lost ${formatMoney(betAmount, config)}`, inline: true }, { name: "New Balance", value: formatMoney(userEconomy.wallet + netChange, config), inline: true })
...
async function handleSlots(interaction, config) {
    ...
    // Log transaction
    await db_1.default.execute(`INSERT INTO gambling_history (guild_id, user_id, game_type, bet_amount, win_amount, result)
         VALUES (?, ?, 'slots', ?, ?, ?)`, [interaction.guild.id, interaction.user.id, betAmount, winAmount, `${slot1}${slot2}${slot3}`]);
    await db_1.default.execute(`INSERT INTO economy_transactions (guild_id, user_id, transaction_type, amount, balance_before, balance_after, description)
         VALUES (?, ?, 'gamble', ?, ?, ?, ?)`, [
        interaction.guild.id,
        interaction.user.id,
...
async function handleBlackjack(interaction, config) {
    ...
    // Log transaction
    await db_1.default.execute(`INSERT INTO gambling_history (guild_id, user_id, game_type, bet_amount, win_amount, result)
         VALUES (?, ?, 'blackjack', ?, ?, ?)`, [interaction.guild.id, interaction.user.id, betAmount, winAmount, `P:${playerTotal} D:${dealerTotal}`]);
    await db_1.default.execute(`INSERT INTO economy_transactions (guild_id, user_id, transaction_type, amount, balance_before, balance_after, description)
         VALUES (?, ?, 'gamble', ?, ?, ?, ?)`, [
        interaction.guild.id,
        interaction.user.id,
    ...
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: "👤 Your Hand", value: `Cards: ${playerCards.join(", ")}\n**Total: ${playerTotal}**`, inline: true }, { name: "🤵 Dealer Hand", value: `Cards: ${dealerCards.join(", ")}\n**Total: ${dealerTotal}**`, inline: true })
        .addFields({ name: "Bet Amount", value: formatMoney(betAmount, config), inline: true }, { name: "Result", value: won ? `✅ Won ${formatMoney(winAmount, config)}` : `❌ Lost ${formatMoney(betAmount, config)}`, inline: true }, { name: "New Balance", value: formatMoney(userEconomy.wallet + netChange, config), inline: true })
        .setTimestamp();
    await interaction.reply({ embeds: [embed] });
}
async function handleRoulette(interaction, config) {
    ...
    // Log transaction
    await db_1.default.execute(`INSERT INTO gambling_history (guild_id, user_id, game_type, bet_amount, win_amount, result)
         VALUES (?, ?, 'roulette', ?, ?, ?)`, [interaction.guild.id, interaction.user.id, betAmount, winAmount, `${number} (${betType})`]);
    await db_1.default.execute(`INSERT INTO economy_transactions (guild_id, user_id, transaction_type, amount, balance_before, balance_after, description)
         VALUES (?, ?, 'gamble', ?, ?, ?, ?)`, [
        interaction.guild.id,
        interaction.user.id,
