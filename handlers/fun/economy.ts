...
interface EconomyConfig {
    ...
    work_min_amount: number;
    work_max_amount: number;
    work_cooldown: number;
    crime_min_amount: number;
    crime_max_amount: number;
    crime_success_rate: number;
    crime_fine_multiplier: number;
    rob_min_amount: number;
    rob_max_amount: number;
    rob_success_rate: number;
    rob_fine_amount: number;
...
interface UserEconomy {
    ...
    last_weekly: Date | null;
    last_work: Date | null;
    last_crime: Date | null;
    last_rob: Date | null;
    created_at: Date;
}
...
async function getOrCreateUserEconomy(guildId: string, userId: string, config: EconomyConfig): Promise<UserEconomy> {
    const [[user]] = await db.execute<UserEconomy[]>(
    ...
    await db.execute(
        `INSERT INTO user_economy (guild_id, user_id, wallet, bank, bank_capacity)
         VALUES (?, ?, ?, 0, ?)`,
        [guildId, userId, config.starting_balance, config.starting_balance * 10]
    );

    const [[newUser]] = await db.execute<UserEconomy[]>(
        'SELECT * FROM user_economy WHERE guild_id = ? AND user_id = ?',
        [guildId, userId]
    );
...
export async function handleBalance(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: '💵 Wallet', value: formatMoney(economy.wallet, config), inline: true },
            { name: '🏦 Bank', value: `${formatMoney(economy.bank, config)} / ${formatMoney(economy.bank_capacity, config)} (${bankUsage}%)`, inline: true },
            { name: '💰 Net Worth', value: formatMoney(netWorth, config), inline: true }
        );
...
export async function handleDaily(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    ...
    await db.execute(
        ...
        [totalAmount, totalAmount, interaction.guild!.id, interaction.user.id]
    );
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: 'Base Amount', value: formatMoney(baseAmount, config), inline: true },
            { name: 'Streak Bonus', value: formatMoney(streakBonus, config), inline: true },
            { name: '🔥 Current Streak', value: `${economy.daily_streak} days`, inline: true }
        )
...
export async function handleWeekly(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    const economy = await getOrCreateUserEconomy(interaction.guild!.id, interaction.user.id, config);
...
export async function handleWork(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    const economy = await getOrCreateUserEconomy(interaction.guild!.id, interaction.user.id, config);
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: 'Base Pay', value: formatMoney(amount, config), inline: true },
            { name: 'Streak Bonus', value: formatMoney(streakBonus, config), inline: true },
            { name: '💼 Work Streak', value: `${economy.work_streak + 1} times`, inline: true }
        );

    await interaction.reply({ embeds: [embed] });
}

export async function handleCrime(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    const economy = await getOrCreateUserEconomy(interaction.guild!.id, interaction.user.id, config);
    ...
    if (success) {
        ...
        await db.execute(
            ...
            [actualFine, interaction.guild!.id, interaction.user.id]
        );
...
export async function handleRob(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    ...
    if (success) {
        ...
        await db.execute(
            ...
            [stolenAmount, stolenAmount, interaction.guild!.id, interaction.user.id]
        );

        await db.execute(
            'UPDATE user_economy SET wallet = wallet - ? WHERE guild_id = ? AND user_id = ?',
            [stolenAmount, interaction.guild!.id, target.id]
        );
        ...
        await logTransaction(
            ...
            `Successfully robbed ${target.username}`,
            target.id
        );
        ...
        await logTransaction(
            ...
            `Robbed by ${interaction.user.username}`,
            interaction.user.id
        );
...
export async function handleDeposit(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: '💵 New Wallet Balance', value: formatMoney(economy.wallet - amount, config), inline: true },
            { name: '🏦 New Bank Balance', value: formatMoney(economy.bank + amount, config), inline: true }
        );

    await interaction.reply({ embeds: [embed] });
}

export async function handleWithdraw(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    const amount = interaction.options.getInteger('amount')!;
    const economy = await getOrCreateUserEconomy(interaction.guild!.id, interaction.user.id, config);
...
export async function handleTransfer(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    const target = interaction.options.getUser('user')!;
    const amount = interaction.options.getInteger('amount')!;
    ...
    await db.execute(
        ...
        [amount, amount, interaction.guild!.id, target.id]
    );
    ...
    await logTransaction(
        ...
        `Transferred to ${target.username}`,
        target.id
    );
...
export async function handleLeaderboard(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    const [rows] = await db.execute<LeaderboardEntry[]>(
    ...
    for (let i = 0; i < rows.length; i++) {
        const user: User | null = await interaction.client.users.fetch(rows[i].user_id).catch(() => null);
...
export async function handleTransactions(interaction: ChatInputCommandInteraction, config: EconomyConfig): Promise<void> {
    ...
    const [rows] = await db.execute<Transaction[]>(
        `SELECT * FROM economy_transactions
         ...
         LIMIT ?`,
        [interaction.guild!.id, interaction.user.id, limit]
    );
