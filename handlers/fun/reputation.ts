...
export async function handleGive(interaction: any): Promise<any> {
    ...
    const [[giverRep]] = await db.execute(
        "SELECT last_rep_timestamp FROM reputation WHERE guild_id = ? AND user_id = ?",
        [guildId, giverUser.id]
    );
    ...
    await db.execute(
        "INSERT INTO reputation (guild_id, user_id, rep_points) VALUES (?, ?, 1) ON DUPLICATE KEY UPDATE rep_points = rep_points + 1",
        [guildId, targetUser.id]
    );

    // Update giver's timestamp
    await db.execute(
        "INSERT INTO reputation (guild_id, user_id, last_rep_timestamp) VALUES (?, ?, NOW()) ON DUPLICATE KEY UPDATE last_rep_timestamp = NOW()",
        [guildId, giverUser.id]
    );
...
export async function handleCheck(interaction: any): Promise<any> {
    const targetUser = interaction.options.getUser("user") || interaction.user;
    const guildId = interaction.guild.id;

    const [[rep]] = await db.execute(
        "SELECT rep_points FROM reputation WHERE guild_id = ? AND user_id = ?",
        [guildId, targetUser.id]
    );
