    ...
    for (const [type, typeItems] of Object.entries(groupedItems)) {
        ...
        typeItems.forEach(item => {
            ...
            if (targetUser.id === interaction.user.id) {
                const properties = [];
                if (item.usable)
                    properties.push("🔧 Usable");
                if (item.tradeable)
                    properties.push("🔄 Tradeable");
                if (item.sellable)
                    properties.push("💵 Sellable");
...
async function handleUse(interaction, config) {
    await interaction.deferReply();
    const itemId = interaction.options.getInteger("item_id");
    const quantity = interaction.options.getInteger("quantity") || 1;
    // Get item from user's inventory
    const [[inventoryItem]] = await db_1.default.execute(`SELECT i.*, s.item_name, s.item_emoji, s.item_type, s.usable, s.item_description
         FROM user_inventory i
         JOIN shop_items s ON i.item_id = s.id
         WHERE i.guild_id = ? AND i.user_id = ? AND i.item_id = ?`, [interaction.guild.id, interaction.user.id, itemId]);
    if (!inventoryItem) {
    ...
    switch (inventoryItem.item_name) {
        ...
        case "Lottery Ticket":
            ...
            if (won) {
                ...
                await db_1.default.execute("UPDATE user_economy SET wallet = wallet + ?, total_earned = total_earned + ? WHERE guild_id = ? AND user_id = ?", [winAmount, winAmount, interaction.guild.id, interaction.user.id]);
                const [[userEconomy]] = await db_1.default.execute("SELECT wallet FROM user_economy WHERE guild_id = ? AND user_id = ?", [interaction.guild.id, interaction.user.id]);
                await db_1.default.execute(`INSERT INTO economy_transactions (guild_id, user_id, transaction_type, amount, balance_before, balance_after, description, related_item_id)
                     VALUES (?, ?, 'reward', ?, ?, ?, ?, ?)`, [
        ...
        case "Gift Box":
            // Random item reward
            const [availableItems] = await db_1.default.execute(`SELECT id, item_name, item_emoji FROM shop_items
                 WHERE (guild_id = ? OR guild_id IS NULL)
                 AND is_active = TRUE
                 AND price BETWEEN 50 AND 1000
                 ORDER BY RAND() LIMIT 1`, [interaction.guild.id]);
            if (availableItems.length > 0) {
                const randomItem = availableItems[0];
                const randomQuantity = Math.floor(Math.random() * 3) + 1;
                // Add random item to inventory
                const [[existingItem]] = await db_1.default.execute("SELECT * FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, interaction.user.id, randomItem.id]);
                if (existingItem) {
                    await db_1.default.execute("UPDATE user_inventory SET quantity = quantity + ? WHERE guild_id = ? AND user_id = ? AND item_id = ?", [randomQuantity, interaction.guild.id, interaction.user.id, randomItem.id]);
                }
                else {
                    await db_1.default.execute("INSERT INTO user_inventory (guild_id, user_id, item_id, quantity) VALUES (?, ?, ?, ?)", [interaction.guild.id, interaction.user.id, randomItem.id, randomQuantity]);
                }
        ...
        case "Bank Note":
            ...
            await db_1.default.execute("UPDATE user_economy SET bank_capacity = bank_capacity + 10000 WHERE guild_id = ? AND user_id = ?", [interaction.guild.id, interaction.user.id]);
    ...
    try {
        if (removeItem) {
            if (inventoryItem.quantity === quantity) {
                await db_1.default.execute("DELETE FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, interaction.user.id, itemId]);
            }
            else {
                await db_1.default.execute("UPDATE user_inventory SET quantity = quantity - ?, use_count = use_count + ?, last_used = NOW() WHERE guild_id = ? AND user_id = ? AND item_id = ?", [quantity, quantity, interaction.guild.id, interaction.user.id, itemId]);
            }
        }
        else {
            await db_1.default.execute("UPDATE user_inventory SET use_count = use_count + ?, last_used = NOW() WHERE guild_id = ? AND user_id = ? AND item_id = ?", [quantity, interaction.guild.id, interaction.user.id, itemId]);
        }
        const embed = new discord_js_1.EmbedBuilder()
            .setColor("#9B59B6")
        ...
        logger_1.default.error("[Inventory] Use item error:", { error: error.message, stack: error.stack });
        await interaction.editReply({
            content: "❌ An error occurred while using the item."
        });
    }
}
async function handleEquip(interaction, config) {
    await interaction.deferReply();
    const itemId = interaction.options.getInteger("item_id");
    // Get item from user's inventory
    const [[inventoryItem]] = await db_1.default.execute(`SELECT i.*, s.item_name, s.item_emoji, s.item_type
         FROM user_inventory i
         JOIN shop_items s ON i.item_id = s.id
         WHERE i.guild_id = ? AND i.user_id = ? AND i.item_id = ?`, [interaction.guild.id, interaction.user.id, itemId]);
    if (!inventoryItem) {
    ...
    if (newEquippedStatus) {
        await db_1.default.execute(`UPDATE user_inventory i
             JOIN shop_items s ON i.item_id = s.id
             SET i.is_equipped = FALSE
             WHERE i.guild_id = ? AND i.user_id = ? AND s.item_type = ? AND i.item_id != ?`, [interaction.guild.id, interaction.user.id, inventoryItem.item_type, itemId]);
    }
    // Update equipped status
    await db_1.default.execute("UPDATE user_inventory SET is_equipped = ? WHERE guild_id = ? AND user_id = ? AND item_id = ?", [newEquippedStatus, interaction.guild.id, interaction.user.id, itemId]);
    const embed = new discord_js_1.EmbedBuilder()
...
async function handleGift(interaction, config) {
    await interaction.deferReply();
    ...
    const [[senderItem]] = await db_1.default.execute(`SELECT i.*, s.item_name, s.item_emoji, s.tradeable
         FROM user_inventory i
         JOIN shop_items s ON i.item_id = s.id
         WHERE i.guild_id = ? AND i.user_id = ? AND i.item_id = ?`, [interaction.guild.id, interaction.user.id, itemId]);
    if (!senderItem) {
    ...
    try {
        ...
        if (senderItem.quantity === quantity) {
            ...
            await db_1.default.execute("UPDATE user_inventory SET quantity = quantity - ? WHERE guild_id = ? AND user_id = ? AND item_id = ?", [quantity, interaction.guild.id, interaction.user.id, itemId]);
        }
        // Add to recipient
        const [[recipientItem]] = await db_1.default.execute("SELECT * FROM user_inventory WHERE guild_id = ? AND user_id = ? AND item_id = ?", [interaction.guild.id, recipient.id, itemId]);
        if (recipientItem) {
            await db_1.default.execute("UPDATE user_inventory SET quantity = quantity + ? WHERE guild_id = ? AND user_id = ? AND item_id = ?", [quantity, interaction.guild.id, recipient.id, itemId]);
        }
        else {
            await db_1.default.execute("INSERT INTO user_inventory (guild_id, user_id, item_id, quantity) VALUES (?, ?, ?, ?)", [interaction.guild.id, recipient.id, itemId, quantity]);
        }
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: "Recipient", value: recipient.username, inline: true }, { name: "Quantity", value: `${quantity}`, inline: true })
            .setTimestamp();
        await interaction.editReply({ embeds: [embed] });
        ...
        logger_1.default.error("[Inventory] Gift error:", { error: error.message, stack: error.stack });
        await interaction.editReply({
