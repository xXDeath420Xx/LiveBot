...
export async function handleStart(interaction: any): Promise<any> {
    ...
    if (questions.length === 0) {
        return interaction.editReply({
            content: "❌ No trivia questions found for the selected category and difficulty."
        });
    }
...
export async function sendQuestion(interaction, gameId: any): Promise<any> {
...
export async function endGame(interaction, gameId: any): Promise<any> {
    const game = activeTriviaGames.get(gameId);
    if (!game) return;
    ...
    try {
        ...
        logger.error("[Trivia] Error updating stats:", { error: _error.message  });
    }
...
export async function handleLeaderboard(interaction: any): Promise<any> {
...
export async function checkAchievements(guildId, userId, oldStats, game: any): Promise<any> {
    try {
        const achievements = [];
        ...
        logger.error("[Trivia] Error checking achievements:", { error: _error.message  });
    }
}

export function getCategoryDisplay(category: any): any {
    const categories = {
...
export function getDifficultyDisplay(difficulty: any): any {
    const difficulties = {
        "easy": "🌱 Easy",
        "medium": "⚡ Medium",
        "hard": "🔥 Hard"
...
export async function handleButton(interaction: any): Promise<any> {
