...
class YtDlpExtractor extends BaseExtractor {
    ...
    buildTrack(entry, searchOptions) {
        if (!entry || !entry.webpage_url || !entry.title) return null;
...
async function start() {
    ...
    client.once(Events.ClientReady, async c => {
        ...
        try { await setupSystemJobs(); } catch (e) { console.error(' Error during system job setup:', e); }
        try { startSystemWorker(c, db); } catch (e) { console.error(' Error starting system worker:', e); }
        try { startAnalyticsScheduler(c, db); } catch (e) { console.error(' Error starting analytics scheduler:', e); }
        try { startReminderWorker(c, db); } catch (e) { console.error(' Error starting reminder worker:', e); }
        try { startAnnouncementWorker(c, db); } catch (e) { console.error(' Error starting announcement worker:', e); }
        try { startSocialFeedWorker(c, db); } catch (e) { console.error(' Error starting social feed worker:', e); }
        try { startPollScheduler(c, db); } catch (e) { console.error(' Error starting poll scheduler:', e); }
        try { startTicketWorker(c, db); } catch (e) { console.error(' Error starting ticket worker:', e); }

        if (process.env.IS_MAIN_PROCESS === 'true') {
            try { await scheduleSocialFeedChecks(); } catch (e) { console.error(' Failed to schedule social feed checks:', e); }
            try { await scheduleTicketChecks(); } catch (e) { console.error(' Failed to schedule ticket checks:', e); }
        }

        if (checkGiveaways) setInterval(checkGiveaways, 15 * 1000);
        if (checkPolls) setInterval(checkPolls, 30 * 1000);
        if (syncTwitchSchedules) setInterval(() => syncTwitchSchedules(c), 5 * 60 * 1000);

        try { await startupCleanup(c); } catch (e) { console.error(' Error during startup cleanup:', e); }

        if (process.env.IS_MAIN_PROCESS === 'true') {
            dashboard.start(client);
        }

        try { logger.init(client, db); } catch (e) { console.error(' Error during logger initialization:', e); }
    });
