...
import {apiChecks} from "../utils/api_checks.js";
import {announcementQueue} from "../jobs/announcement-queue";
import {summaryQueue} from "../jobs/summary-queue";
...
export class StreamChecker {
    ...
    constructor(prisma: PrismaClient) {
        this.prisma = prisma;
        this.roleManager = new RoleManager(this.prisma);
    }

    async fetchAndCache(key: string, fetcher: any, ttl: number) {
    ...
    async checkStreams(client) {
        ...
        try {
            ...
            const announcementsInDb = await this.prisma.$executeRaw`
                SELECT a1.*
                FROM announcements a1
                JOIN (SELECT subscription_id, MAX(announcement_id) as max_announcement_id FROM announcements GROUP BY subscription_id) AS a2
            ...
            const guildSettingsList = await this.fetchAndCache("db:guilds", () => this.prisma.guilds.findMany(), 300);
            const guildSettingsMap = new Map(guildSettingsList.map(g => [g.guild_id, g]));

            const channelSettingsList = await this.fetchAndCache("db:channel_settings", () => this.prisma.channel_settings.findMany(), 300);
            const channelSettingsMap = new Map(channelSettingsList.map(cs => [`${cs.guild_id}-${cs.channel_id}`, cs]));

            const teamSettingsList = await this.fetchAndCache("db:twitch_teams", () => this.prisma.twitch_teams.findMany(), 300);
            ...
            for (const streamer of uniqueStreamers) {
                try {
                    ...
                    const liveData = await this.fetchAndCache(cacheKey, async () => {
                    ...
                    try {
                        ...
                        if (guild) {
                            ...
                            if (member) {
                                ...
                                await this.roleManager.handleRole(member, [guildSettings.live_role_id], "add", sub.guild_id);
                            } else {
                            ...
                            try {
                                ...
                                if (guild) {
                                    ...
                                    if (member) {
                                        ...
                                        await this.roleManager.handleRole(member, [guildSettings.live_role_id], "remove", existing.guild_id);
                                    } else {
    ...
    async checkTeams(client) {
        ...
        try {
            const teamSubscriptions = await this.fetchAndCache("db:twitch_teams", () => this.prisma.twitch_teams.findMany(), 600);
            ...
            for (const sub of teamSubscriptions) {
                try {
                    ...
                    const dbSubs = await this.prisma.subscriptions.findMany({
                        include: {
                            streamers: true
                        },
                        where: {guild_id: sub.guild_id, announcement_channel_id: sub.announcement_channel_id, platform: "twitch"}
                    });
